...
Fetch the implementation issue via `spec-track issue fetch <issue-ref>`
...
to `$(workflow project-dir .spec-track-cache/audits/issue-<ref>-audit.md)`
...
Append via `spec-track issue comment` (verb syntax at `<runbook>`'s
...
- Do not call any `spec-track issue` verb other than `fetch` and `comment`.
- Do not write to any local path other than the audit fallback file
  (`$(workflow project-dir .spec-track-cache/audits/issue-<ref>-audit.md)`),
