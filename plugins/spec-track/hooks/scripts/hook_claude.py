...
``hook.py`` as plain functions.

The script is the executable entry point for Claude's hook manifest
(``plugins/spec-track/hooks/hooks.json``). Each hook command invokes this script
    ...
    emit_json(
        {
            "hookSpecificOutput": {
                ...
                "additionalContext": build_session_start_context(config, _plugin_root(), runtime="claude"),
            }
        },
        stdout=stdout,
    )
