...
- **Codex cannot persist env from SessionStart.** `hook_codex.py` records
  session state under `.spec-track-cache/hook-state/`; the
  `../scripts/spec-track` wrapper sources the exports later via
  `CODEX_THREAD_ID`. Do not rely on `os.environ` for the `SPEC_TRACK_*`
  contract on Codex.
- **Placeholders use `{{NAME}}`, not `$NAME`.** Substituted in
  ...
  etc.) is referenced via `{{SPEC_TRACK_RUNBOOK_DIR}}` +
  `{{SPEC_TRACK_ISSUE_PROVIDER}}` pointing at
...
| New on-demand reference doc | `../authoring/runbook/...`; point at it from injected text via `{{SPEC_TRACK_RUNBOOK_DIR}}/...` |
