"""Tests for .spec-track/config.yml loading and validation."""
...
from config import WorkflowConfigError, load_workflow_config  # noqa: E402
...
""".lstrip(),
    ...
    assert config.knowledge.settings["path"] == "wiki/spec-track"
...
def test_missing_config_returns_none_and_can_be_required(tmp_path: Path) -> None:
    ...
    with pytest.raises(WorkflowConfigError, match=".spec-track/config.yml was not found"):
