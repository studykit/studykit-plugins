    ...
    *,
    ...
    if runtime == "codex":
        launcher_block = launcher_block.replace(
            "{{SPEC_TRACK_PLUGIN_ROOT}}", str(_PLUGIN_ROOT)
        )
    rendered = render_template(text, {
        ...
        "SPEC_TRACK_RUNBOOK_DIR": str(runbook_dir),
        "SPEC_TRACK_ISSUE_PROVIDER": issue_kind,
    })
...
def _write_config(project: Path) -> None:
    config_path = project / ".spec-track" / "config.yml"
    ...
    post_tool_event = parse_claude_event_payload(
        {
            ...
            "tool_name": "Read",
            "tool_input": {
                "file_path": "plugins/spec-track/authoring/contracts/issue/task.md"
            },
            "tool_response": {
                "filePath": "plugins/spec-track/authoring/contracts/issue/task.md"
            },
...
def test_session_start_injects_policy_for_configured_project(
    ...
    if runtime == "claude":
        assert "spec-track <script>" in context
    else:
        assert f"\"{_PLUGIN_ROOT}/scripts/spec-track\" <script>.py" in context
        assert "{{SPEC_TRACK_PLUGIN_ROOT}}" not in context
    assert "read-only" in context
    assert "spec-track mustread" in context
...
def test_claude_session_start_appends_workflow_env_file(
    ...
    assert f"export SPEC_TRACK={_PLUGIN_ROOT / 'scripts' / 'spec-track'}" in content
    assert f"export SPEC_TRACK_PLUGIN_ROOT={_PLUGIN_ROOT}" in content
    assert f"export SPEC_TRACK_PROJECT_DIR={tmp_path}" in content
    assert "export SPEC_TRACK_SESSION_ID=claude-shell-session" in content
...
def test_codex_session_start_writes_session_export_file(
    ...
    assert f"export SPEC_TRACK_PLUGIN_ROOT={_PLUGIN_ROOT}" in content
    assert f"export SPEC_TRACK_PROJECT_DIR={tmp_path}" in content
    assert "export SPEC_TRACK_SESSION_ID=codex-shell-session" in content
    assert "AUTHORING_RESOLVER" not in content
...
def test_codex_hook_state_uses_single_file_per_session(
    ...
    state = json.loads(state_file.read_text(encoding="utf-8"))
    assert state["env"]["SPEC_TRACK_SESSION_ID"] == session_id
...
def test_codex_session_start_migrates_legacy_split_state(
    ...
    legacy_env.write_text("export SPEC_TRACK_SESSION_ID=old-session\n", encoding="utf-8")
...
def test_session_start_skips_claude_subagent_payload(
    ...
    content = env_file.read_text(encoding="utf-8")
    assert f"export SPEC_TRACK={_PLUGIN_ROOT / 'scripts' / 'spec-track'}" in content
    assert "export SPEC_TRACK_SESSION_ID=claude-subagent-session" in content
    assert "AUTHORING_RESOLVER" not in content
...
def test_claude_subagent_start_injects_issue_implementer_agent_block(
    ...
    assert claude_main(
        payload={
            ...
            "hook_event_name": "SubagentStart",
            "agent_id": "agent-impl",
            "agent_type": "spec-track:issue-implementer",
        },
        stdout=captured,
    ) == 0
    ...
    assert context == expected_subagent_start_context(
        issue_kind="github", agent_type="spec-track:issue-implementer"
    )
...
def test_user_prompt_caches_issue_and_injects_project_relative_path(
    ...
    assert "- #45 → `.spec-track-cache/issues/45/issue.md` (refreshed)" in context
...
def test_user_prompt_caches_jira_issue_and_injects_snapshot_path(
    ...
    context = payload["hookSpecificOutput"]["additionalContext"]
    assert payload["hookSpecificOutput"]["hookEventName"] == "UserPromptSubmit"
    assert "- TEST-1234 → `.spec-track-cache/issues/TEST-1234/issue.md` (refreshed)" in context
...
def test_claude_pre_write_blocks_github_cache_issue_body_update(
    ...
    issue_file = tmp_path / ".spec-track-cache" / "issues" / "39" / "issue.md"
...
def test_claude_pre_write_blocks_creating_github_cache_issue_body(
    ...
    issue_file = tmp_path / ".spec-track-cache" / "issues" / "40" / "issue.md"
    issue_text = """---
title: New issue
labels:
  - task
...
"""
    ...
    assert f"Target: {issue_file}" in reason
    assert "spec-track issue fetch" in reason
...
def test_provider_issue_cache_body_recognition_dispatches_to_provider_module(
    ...
    _write_config(tmp_path)
    config = hook.workflow_config_for_project(tmp_path)
    ...
    monkeypatch.setattr(hook, "is_github_issue_cache_body_path", fake_github_recognizer)
    monkeypatch.setattr(hook, "is_jira_issue_cache_body_path", unexpected_jira_recognizer)

    target = tmp_path / "provider-owned-layout-is-not-parsed-here.md"

    assert hook.is_provider_issue_cache_body(target, config)
...
def test_claude_pre_write_blocks_github_cache_meta_json(
    ...
    meta_file = tmp_path / ".spec-track-cache" / "issues" / "39" / ".meta.json"
...
def test_session_start_emits_nothing_for_invalid_config(
    ...
    config_path = tmp_path / ".spec-track" / "config.yml"
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config_path.write_text(
        """
version: 1
