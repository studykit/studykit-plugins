...
def test_jira_issue_cache_body_path_recognizer_matches_issue_md_and_comments(tmp_path: Path) -> None:
    issue_md = tmp_path / ".spec-track-cache" / "issues" / "TEST-1234" / "issue.md"
    comment = tmp_path / ".spec-track-cache" / "issues" / "TEST-1234" / "comment-2026-05-15T093000Z-1.md"
    issue_json = tmp_path / ".spec-track-cache" / "issues" / "TEST-1234" / "issue.json"

    relationships_md = tmp_path / ".spec-track-cache" / "issues" / "TEST-1234" / "relation.md"
    ...
    assert not is_jira_issue_cache_body_path(
        tmp_path / ".spec-track-cache" / "issues" / "TEST-1234" / "metadata.yml", tmp_path
    )
...
def test_jira_issue_cache_paths_are_provider_specific(tmp_path: Path) -> None:
    ...
    assert cache.issue_dir(site, "test-1234") == (
        tmp_path / ".spec-track-cache" / "issues" / "TEST-1234"
    )
