...
def test_github_issue_cache_body_path_recognizer_matches_issue_and_comment_layouts(tmp_path: Path) -> None:
    configured_issue = tmp_path / ".spec-track-cache" / "issues" / "39" / "issue.md"
    configured_comment = tmp_path / ".spec-track-cache" / "issues" / "39" / "comment-2026-05-13T112053Z-4440388606.md"
    external_issue = tmp_path / ".spec-track-cache" / "github.com" / "other" / "repo" / "issues" / "39" / "issue.md"
    external_comment = (
        tmp_path
        / ".spec-track-cache"
        / "github.com"
    ...
    assert not is_github_issue_cache_body_path(
        tmp_path / ".spec-track-cache" / "issues" / "39" / "comments" / "any.md", tmp_path
    )
    assert not is_github_issue_cache_body_path(
        tmp_path / ".spec-track-cache" / "issues-pending" / "draft-1" / "issue.md", tmp_path
    )
...
def test_gitignore_excludes_workflow_cache_root() -> None:
    assert ".spec-track-cache/" in (_REPO_ROOT / ".gitignore").read_text(encoding="utf-8")
...
def test_github_issue_cache_writes_flat_layout_with_meta_and_relationships(tmp_path: Path) -> None:
    ...
    assert write.issue_dir == tmp_path / ".spec-track-cache" / "issues" / "39"
...
def test_configured_repo_cache_is_shallow_and_external_repo_cache_is_namespaced(tmp_path: Path) -> None:
    ...
    assert cache.issue_dir(repo(), 39) == tmp_path / ".spec-track-cache" / "issues" / "39"
    assert (
        cache.issue_dir(external_repo(), 39)
        == tmp_path / ".spec-track-cache" / "github.com" / "other" / "repo" / "issues" / "39"
    )
