...
def _write_config(
    project: Path,
    *,
    knowledge_path: str | None = "wiki/spec-track",
...
def test_resolve_returns_component_path(
    ...
    _write_config(tmp_path, knowledge_path="wiki/spec-track")
    ...
    assert resolution.base == (tmp_path / "wiki/spec-track").resolve()
...
def test_missing_config_raises(tmp_path: Path) -> None:
    with pytest.raises(PrdPathError, match=".spec-track/config.yml was not found"):
        resolve_prd_path("actors", project=tmp_path)
...
def test_to_markdown_output_shape(tmp_path: Path) -> None:
    ...
    expected_base = (tmp_path / "wiki/spec-track").resolve()
    assert output == (
...
def test_prd_path_setting_extends_base(tmp_path: Path) -> None:
    _write_config(tmp_path, knowledge_path="wiki/spec-track", prd_path="prd")

    resolution = resolve_prd_path("actors", project=tmp_path)

    assert resolution.base == (tmp_path / "wiki/spec-track/prd").resolve()
    assert resolution.relative == "usecases/actors.md"


def test_empty_prd_path_uses_knowledge_path_root(tmp_path: Path) -> None:
    _write_config(tmp_path, knowledge_path="wiki/spec-track", prd_path="")
...
def test_resolve_all_returns_every_component_sorted(tmp_path: Path) -> None:
    ...
    assert listing.base == (tmp_path / "wiki/spec-track").resolve()
...
def test_resolve_all_respects_prd_path_setting(tmp_path: Path) -> None:
    ...
    assert listing.base == (tmp_path / "wiki/spec-track/prd").resolve()
