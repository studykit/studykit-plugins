...
``test_cache_*.py`` modules; this file focuses on the routing
...
def _write_config(project: Path, body: str) -> None:
    cfg = project / ".spec-track" / "config.yml"
...
def test_fetch_without_config_emits_setup_hint(tmp_path: Path) -> None:
    # tmp_path has no .spec-track/config.yml
    code, _, err = _capture_help(
        issue_fetch_main,
        ["--project", str(tmp_path), "123"],
    )
    assert code == 2
    assert ".spec-track/config.yml" in err
