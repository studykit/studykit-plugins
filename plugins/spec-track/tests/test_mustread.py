...
def _config_path(project: Path) -> Path:
    path = project / ".spec-track" / "config.yml"
...
def test_bare_review_resolution_uses_absolute_authoring_files() -> None:
    ...
    assert all("plugins/spec-track/operator" not in str(path) for path in resolution.files)
...
def test_require_config_fails_when_missing(tmp_path: Path) -> None:
    with pytest.raises(ResolverError, match=".spec-track/config.yml was not found"):
...
def resolver_session(monkeypatch: pytest.MonkeyPatch) -> tuple[str, str]:
    ...
    monkeypatch.setenv("SPEC_TRACK_SESSION_ID", _RESOLVER_SESSION_ID)
...
def test_main_without_session_emits_sections_without_persisting(
    ...
    monkeypatch.delenv("CODEX_THREAD_ID", raising=False)
    monkeypatch.delenv("SPEC_TRACK_SESSION_ID", raising=False)
    ...
    hook_state_dir = tmp_path / ".spec-track-cache" / "hook-state"
