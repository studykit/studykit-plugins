...
inline metadata and the bundled `scripts/spec-track` launcher runs it via
`uv run --script`.
...
uv run --with pytest --with python-frontmatter --with PyYAML \
  pytest plugins/spec-track/tests
```
...
# One module
uv run --with pytest --with python-frontmatter --with PyYAML \
  pytest plugins/spec-track/tests/test_workflow_jira_issue_cache.py

# One test
uv run --with pytest --with python-frontmatter --with PyYAML \
  pytest plugins/spec-track/tests -k test_jira_metadata_records_native_state
```
