...
``{{SPEC_TRACK_RUNBOOK_DIR}}`` resolves to
``<plugin_root>/authoring/runbook``; ``{{SPEC_TRACK_ISSUE_PROVIDER}}`` resolves
...
``{{SPEC_TRACK_PLUGIN_ROOT}}`` resolved to the absolute plugin root before
...
def build_session_policy_context(
    ...
    *,
    ...
    rendered = render(text, {
        ...
        "SPEC_TRACK_ISSUE_PROVIDER": issue_provider,
    })
...
def _agent_type_segment(agent_type: str | None) -> str | None:
    ...
    if not text:
        return None
    # Strip a plugin namespace prefix (e.g., "spec-track:issue-implementer").
    if ":" in text:
...
def _build_launcher_block(runtime: str | None, plugin_root: Path) -> str:
    ...
    if segment == "codex":
        block = block.replace("{{SPEC_TRACK_PLUGIN_ROOT}}", str(plugin_root))
    return block
