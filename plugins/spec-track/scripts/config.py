...
The workflow plugin uses a repository-root ``.spec-track/config.yml`` even when
...
class WorkflowConfigError(ValueError):
    """Raised when ``.spec-track/config.yml`` is missing or invalid."""
...
def find_workflow_config(project: Path) -> Path | None:
    """Find ``.spec-track/config.yml`` from ``project`` upward."""
...
def load_workflow_config(project: Path, *, require: bool = False) -> WorkflowConfig | None:
    """Load and validate ``.spec-track/config.yml`` for ``project``.
