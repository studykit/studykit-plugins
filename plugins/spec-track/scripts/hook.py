...
from edit_target import EditTarget  # noqa: E402
from config import WorkflowConfig, WorkflowConfigError, load_workflow_config  # noqa: E402
...
from main_context import build_commit_prefix_context  # noqa: E402
from main_context import build_session_policy_context, build_subagent_policy_context  # noqa: E402
...
def provider_cache_body_write_reason(target: EditTarget, config: WorkflowConfig) -> str | None:
    ...
    if is_meta:
        return (
            ...
            "(`spec-track issue update` / `spec-track issue comment --body-file <path>`)."
        )
    ...
    if not target.path.is_file():
        return (
            ...
            "(`spec-track issue fetch`), then use the body-file flow — never edit cache files "
            "directly."
        )
    ...
    return (
        ...
        "workflow dispatcher with `--body-file <path>` (`spec-track issue update` or "
        "`spec-track issue comment`) instead of editing the cached file in place."
    )
