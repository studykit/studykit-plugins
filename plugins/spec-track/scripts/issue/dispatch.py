...
run by the ``spec-track`` launcher). The shared,
provider-agnostic dispatch boilerplate — load ``.spec-track/config.yml``,
...
def run_intent(
    ...
    *,
    ...
    if config is None:
        print(
            f"{spec.label} error: .spec-track/config.yml was not found "
