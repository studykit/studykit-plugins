    ...
    def _apply_state_transition(
        ...
        if not transition_name:
            raise ProviderOperationError(
                ...
                f"Run `setup.py jira-state-transition-inspect --jira-site <url> --issue <KEY>` "
                f"to list the workflow's transitions, then re-run the setup skill so "
                f"providers.issues.state_transitions.{verb} is confirmed into .spec-track/config.yml."
            )
        ...
        if transition_id is None:
            raise ProviderOperationError(
                ...
                f"`setup.py jira-state-transition-inspect --jira-site <url> --issue <KEY>` and "
                f"configure providers.issues.state_transitions.{verb} in .spec-track/config.yml."
            )
...
def create_remote_link(
    ...
    *,
    ...
    if application_type or application_name:
        payload["application"] = {
            "type": application_type or "studykit.spec-track",
