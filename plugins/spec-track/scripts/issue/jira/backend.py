    ...
    def _patch_parser_for_friendly_verb_errors(
        ...
        def hint(verb: str) -> str:
            ...
            return (
                ...
                f"in .spec-track/config.yml (see `setup.py build-config "
        ...
        if verb == "assign":
            ...
            if not native:
                raise IssueBackendError(
                    ...
                    f"Configure providers.issues.artifact_issue_types.{artifact} in .spec-track/config.yml."
                )
            payload = {
                "issue": issue_key,
                "freshness_check": True,
...
def _load_jira_config(project: Path) -> WorkflowConfig:
    ...
    try:
        ...
        raise IssueBackendError(str(exc)) from exc
    if config is None:
        raise IssueBackendError(".spec-track/config.yml was not found")
