...
``SPEC_TRACK_*`` environment contract consumed by workflow script entrypoints.
...
SPEC_TRACK_PLUGIN_ROOT_ENV = "SPEC_TRACK_PLUGIN_ROOT"
SPEC_TRACK_PROJECT_DIR_ENV = "SPEC_TRACK_PROJECT_DIR"
SPEC_TRACK_SESSION_ID_ENV = "SPEC_TRACK_SESSION_ID"
SPEC_TRACK_LAUNCHER_ENV = "SPEC_TRACK"
...
_EXPORT_ORDER = (
    SPEC_TRACK_LAUNCHER_ENV,
    SPEC_TRACK_PLUGIN_ROOT_ENV,
    SPEC_TRACK_PROJECT_DIR_ENV,
    SPEC_TRACK_SESSION_ID_ENV,
)
...
def workflow_env_values(
    *,
    ...
    return {
        SPEC_TRACK_LAUNCHER_ENV: str(resolved_plugin_root / "scripts" / "spec-track"),
        SPEC_TRACK_PLUGIN_ROOT_ENV: str(resolved_plugin_root),
        SPEC_TRACK_PROJECT_DIR_ENV: str(project_dir.expanduser().resolve()),
        SPEC_TRACK_SESSION_ID_ENV: session_id,
    }
...
def codex_env_file_path(project: Path, session_id: str) -> Path:
    ...
    from session_state import session_env_state_path
...
def codex_env_exports(project: Path, session_id: str) -> str:
    ...
    from session_state import read_session_env_exports
...
def write_codex_env_file(
    *,
    ...
    from session_state import record_session_env_exports
...
def workflow_project_dir_from_env(
    *,
    ...
    """Resolve the workflow project directory from ``SPEC_TRACK_PROJECT_DIR`` or a default."""

    env = environ or os.environ
    value = env.get(SPEC_TRACK_PROJECT_DIR_ENV)
    if value:
...
def workflow_session_id_from_env(*, environ: Mapping[str, str] | None = None) -> str:
    """Resolve the normalized workflow session id from ``SPEC_TRACK_SESSION_ID``."""

    env = environ or os.environ
    value = env.get(SPEC_TRACK_SESSION_ID_ENV)
    if value:
        return value
    raise WorkflowEnvError(f"session id is required; pass --session or set {SPEC_TRACK_SESSION_ID_ENV}")
...
def detect_shell_runtime(*, environ: Mapping[str, str] | None = None) -> ShellRuntime:
    ...
    sessions already receive the persisted ``SPEC_TRACK_*`` contract, so callers
