...
Per-project configuration lives at `.spec-track/config.yml` at the repository
...
and writes `.spec-track/config.yml`.
...
`../../wiki/spec-track/spec-track-configuration.md`.

## Slash Commands

Once configured, the plugin exposes these slash commands:

- `/setup` — Generate and write `.spec-track/config.yml` for the current
  repository.
...
- Provider model and reference formats:
  ...
  `../../wiki/spec-track/spec-track-provider-reference-formats.md`
- Issue relationship policy:
  `../../wiki/spec-track/spec-track-issue-relationship-policy.md`
