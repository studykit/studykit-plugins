...
The actors authoring contract is bundled with the usecase knowledge authoring contract returned by `spec-track mustread --type usecase --side knowledge`. Resolve the actors page path with `spec-track prd_path actors`. Cite each `Actors` entry by canonical name with the per-flow role on the curated page. Do not define new actor identity on this curated page; the registry is the only place identity lives.
