...
1. Resolve the `review` authoring docs through the `<authoring-resolver>`:
   ...
   ```bash
   spec-track mustread --type review
   ```
...
3. Publish via `spec-track issue new --type review` (verb syntax at `<runbook>`'s `issue-new` intent). Pass:
