...
When running `spec-track issue link` to apply a Jira relationship, supply the source issue key, canonical relationship intent, and target reference.
...
Do not duplicate Jira-native hierarchy, issue links, or remote links in issue body sections. Read `./anti-patterns.md` for forbidden body sections and fallback boundaries.

Apply provider relationship changes through `spec-track issue link`. The main assistant supplies the relationship intent and target references; the script owns provider invocation and cache refresh.
