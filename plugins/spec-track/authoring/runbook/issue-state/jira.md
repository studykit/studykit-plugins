...
```bash
spec-track issue state <KEY> <verb> [--comment <text>]
spec-track issue assign <KEY> me
spec-track issue unassign <KEY>
spec-track issue set-type <KEY> <new-type>
```

`<verb>` for `state` is dynamic. Run `spec-track issue state --help` to
...
mapping. Use `spec-track issue update` when the change must also rewrite
