...
3. **Link siblings**. After each child is published, link them
   pairwise as `--related` via `spec-track issue link` (verb syntax at
...
3. On the original, append a comment via `spec-track issue comment`
   (verb syntax at `<runbook>`'s `issue-comment` intent) explaining
   the split decision and naming the child refs.
4. Update the original's body via `spec-track issue update` (verb
