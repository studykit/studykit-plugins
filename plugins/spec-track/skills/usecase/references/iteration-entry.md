...
1. Run `spec-track issue list --type usecase` (verb syntax at
...
- **Preserve the audit trail.** Append a comment to the use case
  ...
  `spec-track issue update` to apply the body change. Two operations,
  in that order.
- **Resolve `review` items by updating their state** through
  `spec-track issue update --state closed` (or the equivalent state
