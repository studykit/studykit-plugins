...
- **Reviewer** — `Agent(subagent_type: "spec-track:usecase-reviewer")`.
...
- **Explorer** — `Agent(subagent_type: "spec-track:usecase-explorer")`.
  Pass the list of `usecase` issue refs and the absolute output path
  for the advisory exploration report.
- **Mock generator** — `Agent(subagent_type: "spec-track:mock-html-generator")`.
