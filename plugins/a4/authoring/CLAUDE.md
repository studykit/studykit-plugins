...
- **Skill runtimes also read this directory.** Skills cite `authoring/` for the frontmatter / body contract — never the reverse. Each skill's own orchestration (preconditions, step list, iterate procedure) lives in its own `SKILL.md` + `references/`, not here.
...
- Tooling-managed common field shared by every type (e.g., new auto-stamped timestamp) → extend `frontmatter-common.md`.
...
- Wiki-only body rule (sections used by `actors` / `architecture` / `ci` / `context` / `domain` / `nfr`) → extend `wiki-body.md`.
