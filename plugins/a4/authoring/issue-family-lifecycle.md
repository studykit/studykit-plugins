...
```
open      → discarded | queued | progress | complete
queued    → discarded | progress
progress  → complete | discarded | failing | queued | holding
holding   → progress | discarded
complete  → discarded | queued
...
| Family | Notes |
|---|---|
| `task` | Default family; `cycle:` bumps on `failing → queued`; `complete` means unit tests passed. |
...
| `research` | No `cycle:`; `complete` means findings written and citable; failure paths are usually `failing → queued` for scope re-framing. |
