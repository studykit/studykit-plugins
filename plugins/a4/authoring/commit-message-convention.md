...
| Multi-review cycle findings | `#57 #58 #59 chore(a4): cycle 2 test-runner findings` |
...
`git log --grep="#42"` should return artifact 42's full lifecycle (authoring → implementation → integration → ship → revision). That thread only stays intact if id-prefixing keys off **derivation**, not file-touch — per-task implementation commits typically touch project code only and would drop out under a file-touch trigger. With worktree branches auto-named `worktree-agent-<hex>`, the merge commit's `#<id>` is the only durable task→branch mapping.
