...
- Wiki authoring (`actors`, `architecture`, `ci`, `context`, `domain`, `nfr`) — wikis are not implementation handoffs; the evidence-readiness lens does not apply.
- `usecase`, `spec`, `epic`, `idea`, `brainstorm`, `review` — none are implementer-facing in the same sense; their own authoring contracts cover their evidence-shape.
