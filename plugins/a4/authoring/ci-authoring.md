...
There is exactly one ci page per workspace: `a4/ci.md`. There is no per-topic / per-slug variant.
...
- `## Change Logs` — append-only audit trail of why this page was edited (dated bullets with markdown links to the causing review item or architecture spec). Format and discipline: `./wiki-body.md`.
...
`## How to run tests` is the single executable contract that drives implementation verification. Duplicating it into `architecture.md` (Test Strategy section) would create drift. Architecture's `## Test Strategy` describes the **strategy** (which tiers, what isolation philosophy, what the framework family is); ci's `## How to run tests` records the **executable contract** (the actual commands that exited 0). The two are complementary.

## Change Log triggers

Most bullets that land on this page cite either a `target: architecture` review item that triggered a re-derive, or a test-strategy adjustment (new tier, new runner, isolation change).
...
- **Per-task content leaking in.** Per-task verification belongs in the task's `## Unit Test Strategy` section, not in ci.md.

## Don't

- **Don't write commands that have not been verified.** ci.md records what *succeeded*; speculative commands belong in a spec or research artifact, not here.
- **Don't duplicate `## How to run tests` content into `architecture.md`.** It references, it does not duplicate.
- **Don't write architecture rationale here.** The why behind the test strategy lives in `architecture.md` and the spec(s) that shaped it. ci.md records the *how to run it*.
