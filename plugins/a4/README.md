...
## Migration: `task.files:` → `task.artifacts:` (a4 v10.0.0)

a4 v10.0.0 renamed the task frontmatter key `files:` to `artifacts:` to match the body section name (`## Files` for source paths) and the `artifacts/task/<kind>/<id>-<slug>/` directory layout. The validator now reads only `artifacts:` — any task still carrying `files:` is silently ignored by the artifact-path / completion preflight rules.

```bash
# Rewrite frontmatter key in every task markdown.
#    macOS sed; for GNU sed drop the empty -i argument.
find a4/task -name '*.md' -print0 \
  | xargs -0 sed -i '' -E 's|^files:|artifacts:|'
...
The match is anchored at column 0 so body text containing the literal `files:` (e.g., inside a fenced block) is unaffected.
