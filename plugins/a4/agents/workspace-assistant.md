...
The frontmatter contract — required fields, enums, status meanings, allowed transitions, reverse-link semantics, path-reference format, title placeholders — lives across [`authoring/frontmatter-universals.md`](${CLAUDE_PLUGIN_ROOT}/authoring/frontmatter-universals.md) (universal rules) and [`authoring/<type>-authoring.md`](${CLAUDE_PLUGIN_ROOT}/authoring/) (per-type field tables and lifecycles). Read them on first invocation that needs more than a trivial filter or any transition, then rely on them for filter validity, transition legality, and result interpretation. Validator output (`/a4:validate`, Stop-hook violations) is self-explanatory — each message names the file, field, rule, and (where applicable) the recovery hint.
