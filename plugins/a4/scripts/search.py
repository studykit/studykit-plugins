...
KIND_BY_FOLDER: dict[str, frozenset[str]] = {
    ...
    **{fam: frozenset({fam}) for fam in ISSUE_FAMILY_TYPES},
}
...
class Record:
    ...
    def kind(self) -> str | None:
        ...
        # Task issue families (task/bug/spike/research): the folder
        # doubles as the kind after the v12 split — there is no
        # ``kind:`` field. Other folders: legacy ``kind:`` if present.
        if self.folder == "wiki":
            return _str(self.fm.get("type"))
        if self.folder in ISSUE_FAMILY_TYPES:
            return self.folder
