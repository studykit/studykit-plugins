...
When introducing a new shape, removing one, or changing how an existing shape's required wiki / AC source works, read this document **before** SKILL.md edits. This is the "which path through the pipeline is in flight" reference; `wiki-authorship.md` is the "who can write what" reference.
