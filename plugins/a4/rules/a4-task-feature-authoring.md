...
description: Authoring rules for a4 feature tasks. Auto-loaded when reading or editing anything under `a4/task/feature/`.
paths: ["a4/task/feature/**/*.md"]
---

# a4 — feature task authoring guide

A feature task at `a4/task/feature/<id>-<slug>.md` is a **unit of
regular implementation work** — new functionality, extension, or
refactor. The default task kind in the Jira sense.

Lifecycle is identical across task kinds (`feature` / `bug` / `spike`).
Tasks are produced by `/a4:task` (single ad-hoc) or `/a4:roadmap`
(UC-batch); they are consumed by `/a4:run` (the implement + test loop)
and the `task-implementer` agent.
...
body validation, `implements:` / `spec:` resolution, and the
`implemented_by:` reverse-link refresh all run through the same code
path.

- **`/a4:task`** — single ad-hoc feature task outside the UC-batch
  path. Use for a one-off feature that lands after the roadmap, or for
  post-hoc complete documentation of code already shipped.
...
- **`/a4:task discard <id-or-slug> [reason]`** — explicit one-off
  ...
  discards (when a UC flips to `discarded`) happen automatically — do
...
- `kind: feature` is fixed for files under `a4/task/feature/`. Every
  task must declare the kind explicitly.
- `implements:` lists `usecase/<id>-<slug>` paths the task delivers.
  Declare it whenever the project is UC-driven.
- `spec:` lists `spec/<id>-<slug>` paths backing the task. Declare it
  in UC-less projects (the spec's `decision:` + relevant
  `architecture.md` section becomes the AC source).
...
`spike` (and move the file to `a4/task/spike/`) if the work is
genuinely exploratory, or attach an anchor.
...
- `pending` — In the work queue, awaiting an implementer. Default
...
- `open → progress` is allowed (e.g., `task-implementer` spawned
  outside `/a4:run`). The `pending` step expresses queue intent;
...
1. For each path in `files:`, confirm it exists in the working tree.
   If any path is missing, halt and ask: (a) fix the path, or
   (b) downgrade to `pending` so the task enters the implement loop.
...
3. After writing the file, append an explicit `<log>` block recording
   the post-hoc origin (the writer never logged a
   `progress → complete` transition for this task):
...
- `<acceptance-criteria>` — checklist. AC source:
  ...
  Validators do not enforce source-by-shape — this is a documentation
...
- **`kind:` value mismatched against folder** — a file under
  `a4/task/feature/` must declare `kind: feature`. Mismatched
...
- **Don't author a `kind: spike` or `kind: bug` task here.** Move
  spikes to `a4/task/spike/` and bugs to `a4/task/bug/` so the
