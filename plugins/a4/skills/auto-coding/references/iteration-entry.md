...
- **Merge-sweep retry** (parallel mode only) — for tasks left at `failing` because the merge sweep hit a conflict, the preserved worktree branch is the user's resolution surface. After the user resolves, `/a4:auto-coding iterate parallel` re-attempts `git merge --no-ff` on that branch from local main; on success it transitions the task to `complete` and runs the standard 3-step worktree cleanup. If the user instead discards the work, the task drops back to `queued` and the next cycle re-spawns a fresh worktree. Default serial iterate has no merge-sweep step; failing tasks resume by re-spawning the coder on the dirty working tree (user resolves residue first per `./serial-mode.md`).
