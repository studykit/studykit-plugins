...
This document is the routing table the SKILL.md references. Task implementation is contract-bound to `architecture.md` and AC-bound to `usecase/*.md`, so an upstream finding **stops** the run rather than retrying against stale assumptions.

## Routing table

| Category | What it means | Action | Routing |
|---|---|---|---|
| **task** | Coding error, missing logic, or batch-level oversight (task scope wrong, depends_on missing, AC ambiguous within the task body) | Revise the affected task file(s): update `## Description` / `## Change Plan` (when present) / `## Acceptance Criteria` / `depends_on`. Reset the task `status: queued` by editing the frontmatter directly. Increment `cycle:`. Reset transitively-affected downstream tasks to `queued`. When task revisions are complete, return to Step 1. If the issue spans multiple tasks, leave review items open for a separate task-review pass before resuming. | continue (next cycle) |
...
All `status:` mutations on `<type>/<id>-<slug>.md` (where `<type>` ∈ `{task, bug, spike, research}`) are direct frontmatter edits; the PostToolUse cascade hook runs any cross-file cascade. Add a one-line bullet to the optional `## Log` section *before* flipping `status:` if you want a body-level audit trail (the hook does not write `## Log`).
