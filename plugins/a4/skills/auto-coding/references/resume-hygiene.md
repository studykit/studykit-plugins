...
At session start, for every task with `status: progress`, edit the frontmatter `status:` from `progress` to `queued` directly (a `progress` status at session-start means the prior session crashed mid-work).
