...
`bootstrap.md` is the single source of truth for Launch & Verify (build / launch / test commands, smoke scenario, test isolation flags). Per [`references/wiki-authorship.md`](${CLAUDE_PLUGIN_ROOT}/references/wiki-authorship.md), `roadmap.md` does not author L&V content — it embeds bootstrap's sections via Obsidian transclusion (`![[bootstrap#Verified Commands]]` etc.). Roadmap planning therefore does **not** auto-detect app type, build commands, smoke scenarios, or test isolation; that work happened in `/a4:auto-bootstrap` and was already verified there.
