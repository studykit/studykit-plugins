...
- **After Step 3a (each compose)** — UC files + wiki pages + any `kind: question` review items:
  ```
  #<uc-ids> [#<question-review-ids>] docs(a4): extract-usecase compose growth <N>
  ```
  Body bullet: `- UCs: <total> (<added> added)`.

- **After each quality round** — reviewer-emitted review items + reviser edits to UC files:
  ```
  #<revised-uc-ids> #<written-review-ids> docs(a4): extract-usecase growth <N> review <round>
  ```
...
- **After Step 3d exploration** — exploration report + any new `kind: gap` review items:
  ```
  #<gap-review-ids> docs(a4): extract-usecase exploration growth <N>
  ```
...
- **Final** — summary commit if anything remains unstaged. Subject form depends on what's staged; default to `chore(a4): extract-usecase wrap up` when only ambient updates remain.
