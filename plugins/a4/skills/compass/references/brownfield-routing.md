...
> - **(b) Single change** (Minimal shape) — make one small change without the full pipeline (`bootstrap.md` only, then `/a4:task` or `/a4:bug` → implement step (direct `pending → progress → complete` walk or `/a4:run`)).
> - **(c) New feature** (Full shape) — start the formal pipeline (`/a4:usecase` → `/a4:domain` → `/a4:arch` → `/a4:auto-bootstrap` → `/a4:breakdown` → implement step (direct or `/a4:run`)) for a new feature on top.
...
Full pipeline: `/a4:usecase → /a4:domain → /a4:arch → /a4:auto-bootstrap → /a4:breakdown → implement step` (direct `pending → progress → complete` walk or `/a4:run`; per [`pipeline-shapes.md`](${CLAUDE_PLUGIN_ROOT}/workflows/pipeline-shapes.md) Shape 1 Full forward). Fall through to the Step 2.1 catalog; the user typically picks `/a4:usecase` first.
