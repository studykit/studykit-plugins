...
If either signal fires **and** `a4/` is empty (or absent), the project is **brownfield** — existing code with no a4 workspace. The user must pick one of three pipeline shapes (full taxonomy: [`pipeline-shapes.md`](${CLAUDE_PLUGIN_ROOT}/docs/pipeline-shapes.md)).
...
Full pipeline: `/a4:usecase → /a4:domain → /a4:arch → /a4:auto-bootstrap → /a4:roadmap → /a4:run` (per [`pipeline-shapes.md`](${CLAUDE_PLUGIN_ROOT}/docs/pipeline-shapes.md) Shape 1 Full forward). Fall through to the Step 2.1 catalog; the user typically picks `/a4:usecase` first.
