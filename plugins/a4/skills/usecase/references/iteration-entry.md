...
usecase-specific addendum on top of the shared procedure in [`plugins/a4/references/iterate-mechanics.md`](../../../references/iterate-mechanics.md). This document describes only what the **usecase** iterate flow does between writer calls — research-reflected check, revising-UC scope handling, allowed activities, work-surface summary.
...
Filter open review items to this stage's mailbox: `target: usecase/*` OR `target: context` OR `target: actors` OR `target: nfr` OR same in `wiki_impact`. Present as the priority backlog per [`iterate-mechanics.md`](../../../references/iterate-mechanics.md) §1–§2. The writer-call protocol for **pick → in-progress** and **resolve → resolved** is in §3 of that document.
...
Most discipline is shared (see [`iterate-mechanics.md`](../../../references/iterate-mechanics.md) §5). usecase adds:
