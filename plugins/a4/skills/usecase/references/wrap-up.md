...
4. **Walk findings** — for each emitted review item, present to the user. Resolve in place (edit the target UC / wiki page, then edit the review item's `status:` to `resolved` directly — the PostToolUse cascade hook refreshes `updated:`) or defer (leave `status: open`).
...
The usecase author primary-authors `context.md`, `actors.md`, and `nfr.md` (per `../../../docs/wiki-authorship.md`). When iteration uncovers an issue in `domain.md` or `architecture.md`, **continue** with usecase work and emit a review item targeting the upstream wiki — do not edit it inline.
