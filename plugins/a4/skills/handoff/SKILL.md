...
4. **Verify and record state.** Run the relevant checks when feasible. For an a4 session, the standard verification entry points are:
   - Frontmatter and cross-file consistency: `uv run "${CLAUDE_PLUGIN_ROOT}/scripts/validate.py" "$(git rev-parse --show-toplevel)/a4"` (or `--only frontmatter` / `--only crossref` for a focused run).
   - Project tests / linters when source code shipped this session.

   Record exact commands and outcomes in the handoff. If verification is skipped or fails, say why and include the current failure or risk instead of smoothing it over.
...
Treat `$ARGUMENTS` as extra emphasis or constraints from the user (e.g., "focus on the cycle-3 replan", "include the failing /a4:validate output verbatim"). Incorporate these into the relevant sections rather than tacking them on at the end.
...
- `Validation` — exact commands and outcomes. For a4 sessions, typically includes `uv run "${CLAUDE_PLUGIN_ROOT}/scripts/validate.py" <a4-dir>` and any project-level test runs. Include trailing failure output verbatim when it matters; trim noisy success output.
...
4. The validation summary line (e.g., `validate.py: 0 errors, 0 warnings` or `validate.py: skipped — reason`).
