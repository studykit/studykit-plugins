...
1. **Refresh the `## Resume` snapshot (and append to `## Log` when narrative-worthy) of every mid-flight a4 file this session touched, before anything else.** This is the resume-context step — the next Claude Code session must be able to open any one of those files alone and continue the work without your conversation transcript.
   ...
   - **Compact every slot before writing the new state.** Walk every existing item in every slot and apply the *currently-true* test (in flight right now, or next to do); items that fail (Done sub-steps / shipped phases / settled Open questions / resolved Blocked-on / "_Done._" markers / "settled at …" annotations) must leave the slot in this same edit. If an item carries narrative worth preserving past the current state, append it as a `## Log` entry now — never as a residue inside `## Resume`. The full compaction contract lives in `${CLAUDE_PLUGIN_ROOT}/authoring/issue-body.md` § `## Resume` and § Update discipline.
