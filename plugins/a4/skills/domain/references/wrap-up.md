...
- **Fix now** — edit `domain.md` (and any cross-referenced file). Edit the review item's `status:` to `resolved` directly (the PostToolUse cascade hook refreshes `updated:`), and add a dated `## Change Logs` bullet on each modified wiki page.
