...
> **Authoring contracts:** task files this loop reads — `${CLAUDE_PLUGIN_ROOT}/references/feature-authoring.md`, `${CLAUDE_PLUGIN_ROOT}/references/bug-authoring.md`, `${CLAUDE_PLUGIN_ROOT}/references/spike-authoring.md`. UC ship gates against `${CLAUDE_PLUGIN_ROOT}/references/usecase-authoring.md`. Test-runner findings emit reviews per `${CLAUDE_PLUGIN_ROOT}/references/review-authoring.md`. This skill never writes those files directly — `transition_status.py` and the test-runner agent do.

Two stages over the tasks already authored under the four task-family folders (`a4/feature/`, `a4/bug/`, `a4/spike/`, `a4/research/`):
...
Authoring is out of scope: `/a4:roadmap` writes the roadmap + UC-batch feature tasks; `/a4:feature`, `/a4:bug`, `/a4:spike`, `/a4:research` write single ad-hoc tasks. This skill assumes both have already produced the task files it consumes.
...
- `a4/<type>/<id>-<slug>.md` (under `feature/`, `bug/`, `spike/`, or `research/`) — required. The set of executable units this run consumes.
...
- **Implement mode** — any of `a4/feature/`, `a4/bug/`, `a4/spike/`, `a4/research/` has `pending` or `failing` tasks, or no test-runner review items yet reference the current cycle. Run Steps 1–4 in order. (`open` tasks are backlog and intentionally **not** picked up here.)
...
grep -lhr '^status: pending' a4/feature a4/bug a4/spike a4/research 2>/dev/null
grep -lhr '^status: failing' a4/feature a4/bug a4/spike a4/research 2>/dev/null
ls a4/review/*.md | xargs grep -l 'status: open\|target: roadmap\|target: feature/\|target: bug/\|target: spike/\|target: research/'
```

If no tasks exist at all: halt and tell the user to run `/a4:roadmap` (UC-driven batch) or one of `/a4:feature`, `/a4:bug`, `/a4:spike`, `/a4:research` (single ad-hoc) first.
...
## Acceptance Criteria Source by Task Family

The per-type AC-source convention is defined in the matching authoring rule: feature tasks draw AC from UC `## Flow` / `## Validation` / `## Error Handling` (or spec `## Specification` body + `architecture.md` for UC-less features) per `${CLAUDE_PLUGIN_ROOT}/references/feature-authoring.md`; bug tasks draw AC from a reproduction scenario + the regression test pinning the expected behavior per `${CLAUDE_PLUGIN_ROOT}/references/bug-authoring.md`; spike tasks draw AC from the hypothesis + expected result in the spike's own body per `${CLAUDE_PLUGIN_ROOT}/references/spike-authoring.md`. `/a4:run` does not enforce these — automated checks do not block on AC source; only the `## Acceptance Criteria` section's presence is required by the authoring contract.
...
- Do not rebuild Phase 1. `/a4:roadmap` owns roadmap authoring; `/a4:feature`, `/a4:bug`, `/a4:spike`, `/a4:research` own single-task authoring.
