...
This skill follows the **stop on strong upstream dependency** policy at `../../../docs/wiki-authorship.md` §Cross-stage feedback — task implementation is contract-bound to `architecture.md` and AC-bound to `usecase/*.md`, so an upstream finding halts the run rather than retrying against stale assumptions.
