...
Skill-specific addendum on top of `../../../dev/iterate-mechanics.md`. Read that file first.
...
- **Stop on strong upstream** — `target: architecture` and `target: usecase/*` findings halt the run and route to `/a4:arch iterate` or `/a4:usecase iterate` per `../../../dev/wiki-authorship.md` §Cross-stage feedback. The full classification table is at `./failure-classification.md`.
