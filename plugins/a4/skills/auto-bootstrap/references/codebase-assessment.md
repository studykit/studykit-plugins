...
See `../../../docs/pipeline-shapes.md` for shape definitions.
...
- **Minimal shape** (`architecture.md` may be absent) — when invoked directly without architecture authoring (e.g., a brownfield single-change project entering through `/a4:feature` or `/a4:bug`), derive build / launch / test commands from the observed project state plus minimal user input. Produce a `bootstrap.md` that captures the verified L&V commands; do not back-fill an `architecture.md`. The bootstrap wiki page itself is the only required anchor in Minimal shape.
