...
- **Artifacts** — frontmatter `artifacts:` lists artifact paths under `artifacts/feature/<id>-<slug>/`. Typically empty (production source paths go in the body `## Files` table); populate only when the task uses an artifact directory.
...
These are optional and orthogonal — a feature task may declare zero, one, or both.
...
If **both** `implements:` and `spec:` end up empty, ask the user where the AC will be drawn from. A feature task with no AC source is a smell — either point it at a UC, point it at a spec, or downgrade to `spike` (use `/a4:spike` instead) if the work is genuinely exploratory.
...
Required and optional body sections and the AC-source convention are defined in `${CLAUDE_PLUGIN_ROOT}/references/feature-authoring.md` §Body shape. Compose the section content per that contract.
...
Slugify the title (lowercase, hyphenated, drop non-alphanumeric). File path: `a4/feature/<id>-<slug>.md`.

Frontmatter shape, allowed initial statuses (`open | pending | complete`), and the `complete` preflight (path-existence check on `artifacts:`) are defined in `${CLAUDE_PLUGIN_ROOT}/references/feature-authoring.md` §Frontmatter contract / §`complete` initial-status preflight.
...
- **`open`** — *Task `feature/<id>-<slug>` written at `status: open` (backlog). Not yet enqueued for `/a4:run`. Transition `open → pending` via `transition_status.py` when ready to schedule it.*
- **`pending`** — *Task `feature/<id>-<slug>` written at `status: pending`. Run `/a4:run` to start the implement + test loop.*
- **`complete`** — *Task `feature/<id>-<slug>` written at `status: complete` (post-hoc documentation). No `/a4:run` action needed.*
