...
> Tasks ready. Begin the implement step — drive each task directly (`pending → progress → complete` per `${CLAUDE_PLUGIN_ROOT}/authoring/issue-family-lifecycle.md`). Promote tasks `open → pending` (edit `status:` directly) when you are ready for them to be picked up.
