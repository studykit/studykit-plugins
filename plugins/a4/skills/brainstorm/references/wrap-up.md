...
4. **Decide status** — Before allocating an id, determine the initial `status:` by interpreting the user's natural-language signals:
...
6. **Pick a slug** — Short kebab-case, 2–5 words, derived from the session topic (e.g., `caching-strategy-options`, `dashboard-perf-ideas`).
7. **Write the file** — Save with the Write tool to `a4/brainstorm/<id>-<slug>.md`. Create the `a4/brainstorm/` directory if it does not exist. Use the `status:` from step 4.
8. **Report the path** — After writing, report the full file path so the main session can reference it. Mention the recorded status explicitly (e.g., "Saved as `status: discarded` — no further action.").
...
topic: "<the session's framing question>"
...
Brainstorm body shape requires the `## Ideas` section; `## Notes` and `## Change Logs` are optional. Research findings, TODOs, Constraints, Open Questions, etc. are H3+ headings inside `## Ideas` (or `## Notes` if you prefer to separate them). H1 (`# Title`) is forbidden in body — `title:` and `topic:` live in frontmatter.
