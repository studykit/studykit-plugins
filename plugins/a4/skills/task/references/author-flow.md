...
Frontmatter shape, allowed initial statuses (`open | pending | complete`), and the `complete` preflight (path-existence check on `files:` for `feature`/`spike`/`bug`, body-section presence check for `research`) are defined in the per-kind authoring reference that matches `kind` — `../../../references/task-feature-authoring.md`, `../../../references/task-bug-authoring.md`, `../../../references/task-spike-authoring.md`, or `../../../references/task-research-authoring.md` §Frontmatter contract / §`complete` initial-status preflight.
