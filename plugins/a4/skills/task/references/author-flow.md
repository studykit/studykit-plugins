...
- **Artifacts** — frontmatter `artifacts:` lists artifact paths under `artifacts/task/<kind>/<id>-<slug>/`. For `feature` / `bug`, this list is typically empty (production source paths go in the body `## Files` table); populate only when the task uses an artifact directory. For `spike`, every `artifacts:` path lives under `artifacts/task/spike/<id>-<slug>/`. For `research`, typically empty — the body is the deliverable; populate only when the investigation produces ancillary artifacts under `artifacts/task/research/<id>-<slug>/`.
...
Required and optional body sections and the AC-source convention are defined in the per-kind authoring reference that matches `kind` — `../../../references/task-feature-authoring.md`, `../../../references/task-bug-authoring.md`, `../../../references/task-spike-authoring.md`, or `../../../references/task-research-authoring.md` §Body shape. Compose the section content per that contract.
...
Frontmatter shape, allowed initial statuses (`open | pending | complete`), and the `complete` preflight (path-existence check on `artifacts:` for `feature`/`spike`/`bug`, body-section presence check for `research`) are defined in the per-kind authoring reference that matches `kind` — `../../../references/task-feature-authoring.md`, `../../../references/task-bug-authoring.md`, `../../../references/task-spike-authoring.md`, or `../../../references/task-research-authoring.md` §Frontmatter contract / §`complete` initial-status preflight.
