...
A feature task at `a4/feature/<id>-<slug>.md` is a **unit of regular implementation work** — new functionality, extension, or refactor. The default task type in the Jira sense.
...
Companion to [`./frontmatter-schema.md §Feature task`](./frontmatter-schema.md), `./body-conventions.md`.
...
artifacts: []          # artifact paths under artifacts/feature/<id>-<slug>/ (typically empty)
...
- `type: feature` is fixed for files under `a4/feature/`. There is no `kind:` field — the type *is* the kind.
...
### `type: feature` with empty `implements:` and `spec:` — smell check

A feature task with **both** anchors empty has no AC source. Either downgrade to `spike` (and move the file to `a4/spike/`, changing `type:` to `spike`) if the work is genuinely exploratory, or attach an anchor.
...
- UC-cascade automatic flips: when a UC flips to `discarded`, all related tasks across the four task families → `discarded`. When a UC flips to `revising`, tasks at `progress`/`failing` reset to `pending`; `open`/`pending`/`complete` tasks stay. Do not flip these by hand.
...
- `## Files` — action / path / change table. Lists production source paths the task writes or modifies, plus any artifact paths under `artifacts/feature/<id>-<slug>/` when the task uses an artifact directory.
...
A feature task may have a sibling artifact directory at `<project-root>/artifacts/feature/<id>-<slug>/` when artifacts have evidentiary or comparative value — feature-comparison test samples, execution outputs, design mockups, migration dry-run results:

```
<project-root>/
  a4/feature/<id>-<slug>.md             # task markdown — type: feature
  artifacts/feature/<id>-<slug>/        # comparison samples, outputs, mockups (opt-in)
```
...
Cross-family conventions for the artifact directory — per-type expectations, the `artifacts:` artifact-only contract, what to keep vs. drop, ownership of curation, the project-repo (not scratch) status — live in [`./artifacts.md`](./artifacts.md) and apply to `type: feature` as written there.

## Common mistakes (feature-task-specific)

- **Required section missing** (`## Description`, `## Files`, `## Unit Test Strategy`, `## Acceptance Criteria`).
- **Wrong `type:` value or wrong folder.** A file under `a4/feature/` must declare `type: feature`. Mismatched declarations are a folder-routing error and should be re-located.
...
- **Don't write `kind:` in feature frontmatter.** The field was retired in a4 v12.0.0.
- **Don't author a different task family here.** Move spikes to `a4/spike/`, bugs to `a4/bug/`, and research to `a4/research/` so the matching authoring contract applies.
