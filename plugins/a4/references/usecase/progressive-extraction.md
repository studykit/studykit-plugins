...
When the conversation reveals enough context, draft a Use Case and present it to the user for confirmation. Every UC has five required fields: **Actor**, **Goal**, **Situation**, **Flow**, **Expected Outcome**. Abstraction must stay at the user level — the banned-term list and conversion examples are in `../../skills/usecase/references/abstraction-guard.md`.
...
3. Write `a4/usecase/<N>-<slug>.md` per `../usecase-authoring.md`. Required body sections (per `body_schemas/usecase.xsd`): `<goal>`, `<situation>`, `<flow>` (numbered list), `<expected-outcome>`. Optional: `<validation>`, `<error-handling>`, `<dependencies>`, `<change-logs>`, `<log>` (writer-owned).
