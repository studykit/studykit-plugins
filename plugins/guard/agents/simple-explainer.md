...
description: |
  Explains a topic, file, or a previous answer in plain language, in a clean context. Investigates the repository read-only and returns an explanation a reader understands on first pass — short words, one idea per sentence, the point up front. Dispatched by guard's /guard:explain-simple skill. Never edits files.
