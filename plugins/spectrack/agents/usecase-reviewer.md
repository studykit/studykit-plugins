...
```bash
spectrack mustread --type review --target usecase --side issue
spectrack prd_path actors
```
...
1. Resolve the actors page path with `spectrack prd_path actors`. The
...
Publish the summary via `spectrack issue new --type review`. Pass:
