...
1. **Refresh touched mid-flight issues before anything else.**
   ...
   - For each, draft the new `Resume` section as a current snapshot — Approach / Waiting for / Open questions / Next. Record only the delta against the rest of the body, and point at other sections by name rather than restating them. Remove stale items; the `Resume` section is rewritten in place and does not preserve history.
   - Resolve authoring paths first: `spectrack mustread --type <type>` (add `--side issue|knowledge` for dual-side types) and read the returned files. The provider-write verbs (publish / append / update body-file flow, freshness handling) are documented by `spectrack issue <verb> --help` — read it before drafting.
