...
- The cached `issue.md` and `comment-*.md` projections are read-only. Resume rewrites and comments must go through the configured provider write scripts via the `spectrack` launcher per `${CLAUDE_PLUGIN_ROOT}/authoring/runbook/provider-writes/`. Mutations require a temp body file, user approval, and the freshness-check flow.
