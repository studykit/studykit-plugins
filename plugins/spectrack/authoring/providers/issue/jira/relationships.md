...
When running `spectrack issue link` to apply a Jira relationship, supply the source issue key, canonical relationship intent, and target reference.
...
Apply provider relationship changes through `spectrack issue link`. The main assistant supplies the relationship intent and target references; the script owns provider invocation and cache refresh.
