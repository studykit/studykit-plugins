...
Add purposeful sections when useful. When behavior is observed, prefer explicit `Actual Behavior`, `Expected Behavior`, or `Environment` sections over generic prose. Runtime premises must be runtime-grounded. One optional section carries a bug-specific rule:
