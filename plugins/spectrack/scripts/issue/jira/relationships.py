...
def _issue_link_target_and_label(link: Mapping[str, Any]) -> tuple[Mapping[str, Any] | None, str, str | None]:
    ...
    # the side holding the *other* (far) issue, stored under that far issue's own
    # role. So when the far issue sits in outward_issue, the current issue relates
    # to it via the link's *outward* verb (e.g. the current issue "blocks" it);
    # when the far issue sits in inward_issue, the current issue relates via the
    # *inward* verb (e.g. the current issue "is blocked by" it). Mapping
    # "direction" is keyed on the current/source issue's verb, so return the
    # populated field's own direction. See
