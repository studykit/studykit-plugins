...
def provider_cache_body_write_reason(target: EditTarget, config: WorkflowConfig) -> str | None:
    ...
    if not target.path.is_file():
        return (
            ...
            "(`spectrack issue fetch`), then use the body-file flow — never edit cache files "
            "directly."
        )
    ...
    return (
        ...
        "workflow dispatcher with `--body-file <path>` (`spectrack issue update` or "
        "`spectrack issue comment`) instead of editing the cached file in place."
    )
