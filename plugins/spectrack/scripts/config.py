...
The workflow plugin uses a repository-root ``.spectrack/config.yml`` even when
...
class WorkflowConfigError(ValueError):
    """Raised when ``.spectrack/config.yml`` is missing or invalid."""
...
def find_workflow_config(project: Path) -> Path | None:
    """Find ``.spectrack/config.yml`` from ``project`` upward."""
...
def load_workflow_config(project: Path, *, require: bool = False) -> WorkflowConfig | None:
    """Load and validate ``.spectrack/config.yml`` for ``project``.
