...
def test_render_shell_exports_quotes_values() -> None:
    rendered = render_shell_exports(
        {
            "SPECTRACK": "/tmp/plugin root/scripts/spectrack",
            "SPECTRACK_PLUGIN_ROOT": "/tmp/plugin root",
            "SPECTRACK_PROJECT_DIR": "/tmp/project",
            "SPECTRACK_SESSION_ID": "session-1",
        }
    )

    assert "export SPECTRACK='/tmp/plugin root/scripts/spectrack'" in rendered
    assert "export SPECTRACK_PLUGIN_ROOT='/tmp/plugin root'" in rendered
    assert "export SPECTRACK_PROJECT_DIR=/tmp/project" in rendered
    assert "export SPECTRACK_SESSION_ID=session-1" in rendered
...
def test_append_claude_env_file_prepends_scripts_dir_to_path(tmp_path: Path) -> None:
    ...
    assert f"export SPECTRACK={_SCRIPTS_DIR / 'spectrack'}" in content
...
def test_project_and_session_defaults_from_env(tmp_path: Path) -> None:
    assert workflow_project_dir_from_env(environ={"SPECTRACK_PROJECT_DIR": str(tmp_path)}) == tmp_path.resolve()
    assert workflow_session_id_from_env(environ={"SPECTRACK_SESSION_ID": "workflow-1"}) == "workflow-1"
...
def test_codex_env_file_path_uses_workflow_hook_state(tmp_path: Path) -> None:
    ...
    assert path.parent == tmp_path / ".spectrack-cache" / "hook-state"
...
def test_codex_env_exports_reads_session_state(tmp_path: Path) -> None:
    ...
    assert f"export SPECTRACK={_SCRIPTS_DIR / 'spectrack'}" in rendered
    assert f"export SPECTRACK_PLUGIN_ROOT={_PLUGIN_ROOT}" in rendered
    assert f"export SPECTRACK_PROJECT_DIR={tmp_path}" in rendered
    assert "export SPECTRACK_SESSION_ID=parent-shell" in rendered
...
def test_wrapper_sources_codex_env_file(tmp_path: Path) -> None:
    ...
    script.write_text(
        ...
        "('SPECTRACK', 'SPECTRACK_PLUGIN_ROOT', 'SPECTRACK_PROJECT_DIR', 'SPECTRACK_SESSION_ID', 'AUTHORING_RESOLVER')}))\n",  # AUTHORING_RESOLVER probed to confirm absence
    ...
    proc = subprocess.run(
        [str(_SCRIPTS_DIR / "spectrack"), str(script)],
        check=True,
        cwd=tmp_path,
        env={
    ...
    assert payload["SPECTRACK_PROJECT_DIR"] == str(tmp_path)
    assert payload["SPECTRACK_SESSION_ID"] == "parent-shell"
...
def test_wrapper_does_not_use_codex_thread_id_as_session_fallback(tmp_path: Path) -> None:
    ...
    script.write_text(
        "import os\n"
        "print(os.environ.get('SPECTRACK_SESSION_ID', '<missing>'))\n",
        encoding="utf-8",
    )
...
def test_wrapper_uses_terminal_defaults_without_runtime_marker(tmp_path: Path) -> None:
    ...
    script.write_text(
        "import json, os\n"
        "print(json.dumps({key: os.environ.get(key) for key in "
        "('SPECTRACK', 'SPECTRACK_PLUGIN_ROOT', 'SPECTRACK_PROJECT_DIR', 'SPECTRACK_SESSION_ID')}))\n",
        encoding="utf-8",
    )
    ...
    assert payload["SPECTRACK_PLUGIN_ROOT"] == str(_PLUGIN_ROOT)
    assert payload["SPECTRACK_PROJECT_DIR"] == str(tmp_path)
    assert payload["SPECTRACK_SESSION_ID"] is None


def test_wrapper_resolves_bare_name_to_runnable(tmp_path: Path) -> None:
    proc = subprocess.run(
        [str(_SCRIPTS_DIR / "spectrack"), "issue", "--help"],
        check=True,
        cwd=tmp_path,
        env={"PATH": os.environ.get("PATH", "")},
        stdout=subprocess.PIPE,
...
def test_wrapper_terminal_defaults_use_launcher_plugin_root(tmp_path: Path) -> None:
    ...
    script.write_text(
        "import json, os\n"
        "print(json.dumps({key: os.environ.get(key) for key in "
        "('SPECTRACK', 'SPECTRACK_PLUGIN_ROOT', 'SPECTRACK_PROJECT_DIR')}))\n",
        encoding="utf-8",
    )
    ...
    proc = subprocess.run(
        ...
        env={
            "PATH": os.environ.get("PATH", ""),
            "SPECTRACK": "/tmp/stale/workflow",
            "SPECTRACK_PLUGIN_ROOT": "/tmp/stale/plugin",
            "SPECTRACK_PROJECT_DIR": str(tmp_path / "project-from-env"),
        },
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
