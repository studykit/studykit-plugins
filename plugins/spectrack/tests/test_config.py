"""Tests for .spectrack/config.yml loading and validation."""
...
""".lstrip(),
    ...
    assert config.knowledge.settings["path"] == "wiki/spectrack"
...
def test_missing_config_returns_none_and_can_be_required(tmp_path: Path) -> None:
    ...
    with pytest.raises(WorkflowConfigError, match=".spectrack/config.yml was not found"):
