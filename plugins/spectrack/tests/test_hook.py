    ...
    post_tool_event = parse_claude_event_payload(
        {
            ...
            "tool_name": "Read",
            "tool_input": {
                "file_path": "plugins/spectrack/authoring/contracts/issue/task.md"
            },
            "tool_response": {
                "filePath": "plugins/spectrack/authoring/contracts/issue/task.md"
            },
...
def test_claude_session_start_appends_workflow_env_file(
    ...
    assert f"export SPECTRACK={_PLUGIN_ROOT / 'scripts' / 'spectrack'}" in content
    assert f"export SPECTRACK_PLUGIN_ROOT={_PLUGIN_ROOT}" in content
    assert f"export SPECTRACK_PROJECT_DIR={tmp_path}" in content
    assert "export SPECTRACK_SESSION_ID=claude-shell-session" in content
...
def test_codex_session_start_writes_session_export_file(
    ...
    assert f"export SPECTRACK_PLUGIN_ROOT={_PLUGIN_ROOT}" in content
    assert f"export SPECTRACK_PROJECT_DIR={tmp_path}" in content
    assert "export SPECTRACK_SESSION_ID=codex-shell-session" in content
    assert "AUTHORING_RESOLVER" not in content
...
def test_codex_session_start_migrates_legacy_split_state(
    ...
    legacy_env.write_text("export SPECTRACK_SESSION_ID=old-session\n", encoding="utf-8")
...
def test_session_start_skips_claude_subagent_payload(
    ...
    content = env_file.read_text(encoding="utf-8")
    assert f"export SPECTRACK={_PLUGIN_ROOT / 'scripts' / 'spectrack'}" in content
    assert "export SPECTRACK_SESSION_ID=claude-subagent-session" in content
    assert "AUTHORING_RESOLVER" not in content
...
def test_user_prompt_caches_issue_and_injects_project_relative_path(
    ...
    assert "- #45 → `.spectrack-cache/issues/45/issue.md` (refreshed)" in context
...
def test_user_prompt_caches_jira_issue_and_injects_snapshot_path(
    ...
    context = payload["hookSpecificOutput"]["additionalContext"]
    assert payload["hookSpecificOutput"]["hookEventName"] == "UserPromptSubmit"
    assert "- TEST-1234 → `.spectrack-cache/issues/TEST-1234/issue.md` (refreshed)" in context
...
def test_claude_pre_write_blocks_github_cache_issue_body_update(
    ...
    issue_file = tmp_path / ".spectrack-cache" / "issues" / "39" / "issue.md"
...
def test_claude_pre_write_blocks_creating_github_cache_issue_body(
    ...
    issue_file = tmp_path / ".spectrack-cache" / "issues" / "40" / "issue.md"
    issue_text = """---
title: New issue
labels:
  - task
...
"""
    ...
    assert f"Target: {issue_file}" in reason
    assert "spectrack issue fetch" in reason
...
def test_claude_pre_write_blocks_github_cache_meta_json(
    ...
    meta_file = tmp_path / ".spectrack-cache" / "issues" / "39" / ".meta.json"
...
def test_session_start_emits_nothing_for_invalid_config(
    ...
    config_path = tmp_path / ".spectrack" / "config.yml"
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config_path.write_text(
        """
version: 1
