...
def test_detect_shell_runtime_fails_closed_when_ambiguous() -> None:
    with pytest.raises(WorkflowEnvError, match="ambiguous"):
        detect_shell_runtime(
            environ={
...
def test_workflow_wrapper_sources_codex_env_file(tmp_path: Path) -> None:
    ...
    script.write_text(
        ...
        "('WORKFLOW_PLUGIN_ROOT', 'WORKFLOW_PROJECT_DIR', 'WORKFLOW_SESSION_ID')}))\n",
        encoding="utf-8",
    )
...
def test_workflow_wrapper_fails_closed_for_ambiguous_runtime(tmp_path: Path) -> None:
    ...
    assert "ambiguous shell runtime" in proc.stderr
