...
for explicit commit-only operations. Use
...
For existing cached issue body edits, prefer local projection write-back: use
`.workflow-cache/issues/<number>/issue.md` as the editable draft, then run
`workflow_cache_writeback.py`. Use `workflow_github.py edit-body` only when the
caller explicitly requests a direct provider edit or no cache projection exists.
