...
- GitHub Wiki page: page title, wiki page URL, or provider-native wiki link syntax.
...
GitHub Wikis are separate Git repositories. A wiki can be cloned with a URL like `https://github.com/YOUR-USERNAME/YOUR-REPOSITORY.wiki.git`, and only changes pushed to the wiki repository's default branch are live. Source: https://docs.github.com/en/communities/documenting-your-project-with-wikis/adding-or-editing-wiki-pages

GitHub states that the wiki filename determines the page title and the file extension determines rendering. For example, `.md` and `.markdown` use Markdown rendering. GitHub also warns against characters that are problematic in filenames, including `\ / : * ? " < > |`. Source: https://docs.github.com/en/communities/documenting-your-project-with-wikis/adding-or-editing-wiki-pages

GitHub Wiki content can link to other wiki pages using Markdown links to full wiki page URLs or MediaWiki-style links such as `[[Nameofwikipage|Link Text]]`. Source: https://docs.github.com/en/communities/documenting-your-project-with-wikis/editing-wiki-content

GitHub documents a soft limit of 5,000 total wiki files, after which some pages may be inaccessible. Source: https://docs.github.com/en/communities/documenting-your-project-with-wikis/about-wikis

Implications for workflow:

- Treat the wiki as a `github-wiki` page provider, not as ordinary repository files.
- Store `host`, `owner`, `repo`, `wiki_path`, `title`, and rendered `web_url`.
- The durable provider object is the file path in the wiki Git repository plus its commit history, not a numeric page ID.
- Page titles and URL slugs are human-readable but rename-sensitive.
- There is no obvious GitHub Wiki page-level opaque stable ID comparable to a GitHub issue `node_id`; use path/title plus optional last-seen commit SHA for change detection, not identity.
...
| GitHub Wiki page | Page title or `/wiki/Page-Slug` | host, owner, repo, wiki path, optional commit/version | Rename/path changes change the primary locator; no clear page-level opaque ID. |
...
url: https://github.com/octo-org/octo-repo/wiki/Home
display: Home
```
...
- `provider`: `filesystem`, `github`, `github-wiki`, `jira`, or `confluence`.
...
1. Confirm GitHub Wiki URL slug rules empirically for spaces, Unicode, punctuation, case, and duplicate titles. Official docs explain filename/title coupling but do not fully specify slug normalization.
2. Validate whether GitHub Wiki page renames preserve any redirect behavior in the web UI. Do not assume stable redirects until tested.
3. Decide whether workflow should store GitHub `node_id` for issues as a mandatory field. It is stable and API-friendly, but less portable and less readable than `owner/repo#number`.
