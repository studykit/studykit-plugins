...
- Main-assistant SessionStart wording lives under `./context/main/`: the always-loaded entry point in `session-policy.md`, GitHub knowledge guidance in `policy/knowledge/github.md`, and on-demand detail files under `policy/` (authoring, provider writes); shared runtime/provider snippets live alongside under `./context/snippets/launcher/`, `./context/snippets/issue-fetch/`, and `./context/snippets/issue-write/`.
...
- Claude writes that contract to `CLAUDE_ENV_FILE` when Claude provides it for `SessionStart`, so the main session shell has the `workflow` launcher on `PATH` and the contract env vars exported directly.
...
- For GitHub issue providers, the policy stays narrow: it tells the main assistant to run the workflow `issue` dispatcher rather than raw `gh` for workflow operations, and to follow the publish/append/update body-file contract in `policy/provider-writes.md`.
...
- When the prompt asks for a commit, injects the main-assistant commit guidance from `./context/main/commit-prefix.md` at most once per session.
