...
- The Claude `workflow-operator` agent frontmatter registers a `SubagentStart` hook. When the spawned agent matches `workflow-operator`, `hook_claude.py` injects a config-specific bootstrap context containing the absolute `../scripts/workflow` launcher path, the configured issue command aliases, and configured knowledge-provider guidance.
