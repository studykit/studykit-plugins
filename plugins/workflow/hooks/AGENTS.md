...
`../scripts/workflow_hook.py` and `../scripts/workflow_main_context.py`;
...
- **Adapters are host-specific; shared logic is host-neutral.** Do not add
  Claude/Codex branches inside `../scripts/workflow_hook.py` — push the
...
- **Codex cannot persist env from SessionStart.** `hook_codex.py` records
  ...
  `CODEX_THREAD_ID`. Do not rely on `os.environ` for the `WORKFLOW_*`
  contract on Codex.
...
| Change | Touch |
|---|---|
| New hook event on a runtime | adapter (`hook_claude.py` / `hook_codex.py`) + shared fn in `../scripts/workflow_hook.py` + matching manifest |
...
| New provider/runtime snippet variant | `context/snippets/<group>/<key>.md`; wire the placeholder in `../scripts/workflow_main_context.py` |
| New on-demand reference doc | `../authoring/runbook/...`; point at it from injected text via `{{WORKFLOW_RUNBOOK_DIR}}/...` |
