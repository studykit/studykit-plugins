...
# ///
"""Issue comment append dispatcher.
...
def main(
    ...
    *,
    ...
    try:
        ...
        print(f"issue comment error: {exc}", file=err)
        return 2
    if config is None:
        print(
            "issue comment error: .workflow/config.yml was not found "
    ...
    backend.add_comments_args(parser)
    args = parser.parse_args(argv)

    return backend.run_comments(
        args,
        config=config,
        runner=runner,
        stdout=out,
