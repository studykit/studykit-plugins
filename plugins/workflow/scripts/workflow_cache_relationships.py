...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "issues",
        nargs="+",
        help="configured-provider issue references",
    )
    return parser
...
def stage_relationships_payload(
    *,
    ...
    """Stage relationship intent for an existing configured provider issue."""

    config = _load_issue_config(project)
    if config.issues.kind == "github":
        if related:
            raise WorkflowCacheRelationshipsError("related relationship staging is not supported for GitHub issue providers")
    ...
    if config.issues.kind != "jira":
        raise WorkflowCacheRelationshipsError(
            f"workflow pending relationship staging supports GitHub and Jira issue providers, not {config.issues.kind}"
        )
    ...
    if not cache.has_issue_projection(site, issue_key):
        raise WorkflowCacheRelationshipsError(
            f"Jira issue cache projection is missing for {issue_key}; refresh the issue cache first"
        )
...
def apply_pending_relationships_payload(
    *,
    ...
    """Apply pending local relationship files to configured provider issues."""
    ...
    if config.issues.kind != "jira":
        raise WorkflowCacheRelationshipsError(
            f"workflow pending relationship apply supports GitHub and Jira issue providers, not {config.issues.kind}"
        )
    site, issue_keys = _jira_relationship_context(config, issues)
    results = []
...
def _jira_relationship_context(config, issues: list[str]):
    try:
    ...
    for issue in issues:
        try:
            ...
            raise WorkflowCacheRelationshipsError(str(exc)) from exc
    if not issue_keys:
        raise WorkflowCacheRelationshipsError("no Jira issue references were found")
    return site, issue_keys


def _load_issue_config(project: Path):
    try:
    ...
    *,
    ...
    for item in payload.get("issues", []):
        if isinstance(item, dict):
            ...
            display = f"#{issue}" if payload.get("kind") == "github" else str(issue)
            print(f"{display} applied={item.get('applied')}", file=output)
    return 0
