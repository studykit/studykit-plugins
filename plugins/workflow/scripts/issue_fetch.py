...
def main(
    ...
    *,
    ...
    try:
        ...
        print(f"issue fetch error: {exc}", file=err)
        return 2
    if config is None:
        print(
            "issue fetch error: .workflow/config.yml was not found "
    ...
    backend.add_fetch_args(parser)
    args = parser.parse_args(argv)

    return backend.run_fetch(
        args,
        config=config,
        runner=runner,
        stdout=out,
