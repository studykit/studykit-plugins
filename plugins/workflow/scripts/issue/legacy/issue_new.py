...
def main(
    ...
    *,
    ...
    try:
        ...
        print(f"issue draft error: {exc}", file=err)
        return 2
    if config is None:
        print(
            "issue draft error: .workflow/config.yml was not found "
    ...
    backend.add_drafts_args(parser)
    args = parser.parse_args(argv)

    return backend.run_drafts(
        args,
        config=config,
        runner=runner,
        stdout=out,
