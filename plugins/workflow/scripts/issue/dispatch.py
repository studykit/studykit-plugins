...
run by the ``workflow`` launcher). The shared,
provider-agnostic dispatch boilerplate — load ``.workflow/config.yml``,
...
def run_intent(
    ...
    *,
    ...
    if config is None:
        print(
            f"{spec.label} error: .workflow/config.yml was not found "
