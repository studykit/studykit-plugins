...
def main(
    ...
    *,
    ...
    try:
        ...
        print(f"issue update error: {exc}", file=err)
        return 2
    if config is None:
        print(
            "issue update error: .workflow/config.yml was not found "
    ...
    backend.add_writeback_args(parser)
    args = parser.parse_args(argv)

    return backend.run_writeback(
        args,
        config=config,
        runner=runner,
        stdout=out,
