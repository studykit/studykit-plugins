#!/usr/bin/env python3
"""Codex hook subtype and CLI entry point.
...
This module also doubles as the executable entry point for Codex's hook
manifest (``plugins/workflow/hooks/hooks.codex.json``). Each event command
invokes ``python3 hook_codex.py <subcommand>``; ``main`` dispatches to the
matching shim in :mod:`workflow_hook`.
"""
