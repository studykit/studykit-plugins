...
1. **Refresh touched mid-flight issues before anything else.**
   ...
   - Resolve authoring paths first: `workflow authoring_resolver.py --type <type>` and read the returned files. The provider-write contract (publish / append / update body-file flow, freshness handling) lives at `${CLAUDE_PLUGIN_ROOT}/authoring/runbook/provider-writes/<provider>.md` — open it before drafting.
