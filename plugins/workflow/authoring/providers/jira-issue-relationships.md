...
When running `$WORKFLOW jira_issue_relationships.py` to apply a Jira relationship, supply the source issue key, canonical relationship intent, and target reference.
...
Apply provider relationship changes through `$WORKFLOW jira_issue_relationships.py`. The main assistant supplies the relationship intent and target references; the script owns provider invocation and cache refresh.
