...
(`issue.md` / `relationships.json` / `comment-*.md`), never the internal
`.meta.json`. Reread those paths, reapply your change to the body file,
and retry; or rerun the same verb with `--overwrite` to replace the
provider copy without rereading.
