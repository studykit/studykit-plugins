...
| `claude-code-hook-command-placeholders.md` | That a hook `command` in `settings.json` may reference a project-local script through the `${CLAUDE_PROJECT_DIR}` placeholder (exec form preferred, double-quoted in shell form), and the common input fields every hook event receives — `session_id`, `prompt_id`, `cwd`, `transcript_path` | code.claude.com/docs/en/hooks |
| `claude-code-hooks-session-env.md` | SessionStart env persistence (`$CLAUDE_ENV_FILE`), hook output fields, the matcher `source` values (`startup`/`resume`/`clear`/`compact`/`fork`), and which events show plain stdout to the model; plus a measured note that the env file is a **sourced** shell script, so it also takes a `PATH` prepend or a function definition — and why an executable on `PATH` beats a function (subprocesses inherit it) | code.claude.com/docs/en/hooks |
...
| `claude-code-userpromptexpansion-hook.md` | The `UserPromptExpansion` event: trigger, matcher-on-command-name, input schema (`command_name`, `command_args`, `expansion_type`, `command_source`, `prompt`), and the blocking contract — top-level `decision: "block"` + `reason` (not `permissionDecision`), exit 2 routing stderr the same way, the "turn ends … regardless of `continue`" clause, and which output fields the docs say reach the user vs. Claude; plus what the page leaves unstated about whether any model inference happens on a block | code.claude.com/docs/en/hooks |
...
| `claude-code-plugin-agents-directory-discovery.md` | That a plugin's `agents/` directory is scanned recursively (any `.md` file at any depth is discovered, not only top-level ones), that a subfolder there becomes part of the scoped identifier (`agents/review/security.md` → `plugin:review:security`), and the documented skip rules — no `name`/unparseable YAML skips a project/user agent but a **plugin** agent still loads under its filename, while a `name` starting with `-` or containing `:` is skipped everywhere; also notes the pages state nothing about extension-based filtering | code.claude.com/docs/en/sub-agents |
...
| `claude-code-subagent-resume.md` | Naming a subagent at dispatch, resuming one by name with `SendMessage` (full history retained, auto-resume in background), how long subagent transcripts persist, the interactive panel that lets the **user** open a background agent's transcript and message it directly, `@agent-<plugin>:<name>` as the entry point that *guarantees* a given subagent runs (and that keeps a running one reachable through the typeahead), what decides foreground vs background, and fork vs plain subagent | code.claude.com/docs/en/sub-agents |
