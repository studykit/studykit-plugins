...
- GitHub repository `wiki/` page: repository-relative Markdown path such as `wiki/spectrack/home.md`, or a full GitHub file URL when portability matters.
...
- The GitHub provider context should come from `.spectrack/config.yml` first, then the configured git remote, then `origin`.
- Record `issue_id_format: github` in `.spectrack/config.yml` so hooks can use the configured issue ID format when scanning prompts and hook payloads.
...
- Use normal Markdown links for visible references, such as `[Provider Model](spectrack-provider-model.md)`.
...
- Resolve Jira keys using `.spectrack/config.yml` provider context, especially the Jira `site`.
...
- #123
- PROJ-456
- [Auth Session v2](https://github.com/org/repo/blob/main/wiki/spectrack/auth-session-v2.md)
```

When provider metadata supports structured relationships, store a resolved object. The object can be generated from the raw ref and `.spectrack/config.yml`.
...
native:
  owner: octo-org
  repo: octo-repo
  path: wiki/spectrack/home.md
  title: Home
  last_seen_commit: abc1234
url: https://github.com/octo-org/octo-repo/blob/main/wiki/spectrack/home.md
display: wiki/spectrack/home.md
```
