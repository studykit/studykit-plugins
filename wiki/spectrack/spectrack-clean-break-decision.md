...
Create a new `plugins/spectrack` plugin instead of turning `plugins/a4` into a remote-native provider workflow system.

`plugins/a4` remains the existing local Markdown workflow plugin. `plugins/spectrack` is the clean-break implementation for provider-backed workflow over GitHub, Jira, and repository `wiki/` directories.
...
- `.spectrack/config.yml` at repository root.
