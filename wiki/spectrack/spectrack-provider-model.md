...
See `spectrack-issue-relationship-policy.md` for the GitHub issue relationship policy used by the workflow MVP backlog.
...
The agent should read `.spectrack/config.yml`, determine configured issue and knowledge providers, then query those providers through workflow wrapper read commands when available. Direct provider tools may be used only as an implementation detail until wrapper read commands exist.
...
- Provider, resolved from `.spectrack/config.yml` when possible.
...
13. Remote-native configuration should live in `.spectrack/config.yml`, not `.a4/`.
