...
`plugins/spec-track/authoring/runbook/`.

### Workflow scripts

The bundled workflow scripts under `plugins/spec-track/scripts/` resolve
...
implementation details while editing `plugins/spec-track/`. Keep those details in
source code, tests, or contributor-facing documentation, not in main-agent
runtime guidance.

## Authoring Contract Source

Runtime authoring contracts live under `plugins/spec-track/authoring/`.
...
1. SessionStart injects only the concise workflow policy entry point when
   `.spec-track/config.yml` exists; detail files under
   `plugins/spec-track/authoring/runbook/` are loaded on demand.
