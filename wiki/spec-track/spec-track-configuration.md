...
When the knowledge provider is `github`, workflow uses the main repository `wiki/` directory, such as `wiki/spec-track/`. It does not use GitHub's separate wiki feature.
...
```text
plugins/spec-track/scripts/config.py
```

The loader discovers `.spec-track/config.yml` from the project path upward.
