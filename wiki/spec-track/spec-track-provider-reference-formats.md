...
`plugins/spec-track` should prefer provider-native short references in bodies, commits, branch names, PR titles, and comments. The provider adapter resolves those short references using `.spec-track/config.yml`, git remotes, and provider context.
...
- GitHub repository `wiki/` page: repository-relative Markdown path such as `wiki/spec-track/home.md`, or a full GitHub file URL when portability matters.
...
- The GitHub provider context should come from `.spec-track/config.yml` first, then the configured git remote, then `origin`.
- Record `issue_id_format: github` in `.spec-track/config.yml` so hooks can use the configured issue ID format when scanning prompts and hook payloads.
...
```text
wiki/spec-track/spec-track-provider-model.md
```
...
- Use normal Markdown links for visible references, such as `[Provider Model](spec-track-provider-model.md)`.
...
- Resolve Jira keys using `.spec-track/config.yml` provider context, especially the Jira `site`.
...
- #123
- PROJ-456
- [Auth Session v2](https://github.com/org/repo/blob/main/wiki/spec-track/auth-session-v2.md)
```

When provider metadata supports structured relationships, store a resolved object. The object can be generated from the raw ref and `.spec-track/config.yml`.
...
native:
  owner: octo-org
  repo: octo-repo
  path: wiki/spec-track/home.md
  title: Home
  last_seen_commit: abc1234
url: https://github.com/octo-org/octo-repo/blob/main/wiki/spec-track/home.md
display: wiki/spec-track/home.md
```
