...
Create a new `plugins/spec-track` plugin instead of turning `plugins/a4` into a remote-native provider workflow system.

`plugins/a4` remains the existing local Markdown workflow plugin. `plugins/spec-track` is the clean-break implementation for provider-backed workflow over GitHub, Jira, and repository `wiki/` directories.
...
- `.spec-track/config.yml` at repository root.
