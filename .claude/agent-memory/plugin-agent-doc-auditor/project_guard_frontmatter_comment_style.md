...
`guard/agents/*.md` frontmatter carries long `#` comments above `tools:` and
...
duplicates prose already in `guard/AGENTS.md` under "What guard is".
...
(`guard/AGENTS.md`, `guard/dev/design.md`), not the frontmatter comment
...
**How to apply:** when auditing any `guard/agents/*.md` frontmatter comment,
...
changed* (history, flag as axis-3 finding, fix = move to `guard/AGENTS.md` or
