...
- Whether the live session runs the working tree: `$GUARD_TOGGLE_CLI` points at
  `plugins/guard/scripts/guard_hook.py` under the repo when started with `--plugin-dir`.
...
- What a document dispatch carries: `plugins/guard/scripts/guard_core/cmd_inputs.py`
  `_inputs_for_file` (prints only `file:` + `knowledge dir:`), and
  `plugins/guard/agents/report-router.md` Output (~L156-164, "no turn id and no transcript").
- What a turn dispatch carries: `plugins/guard/hooks/context/dispatch-playbook.md` L128, but the
