...
| `PostToolUse` (`Bash\|Write\|Edit\|MultiEdit\|NotebookEdit`) and `PostToolUseFailure` (`Bash`) | `post-edit` | Record a source file, an agent instruction file, a saved reference or an ordinary document this turn wrote (the candidate lists for `comment-corrector` and document-review rules), then block when a file saved in the refs dir is not listed in that dir's `AGENTS.md`. Native file events record exact targets. Bash writes are found after successful and failed commands by comparing content hashes for reviewable Git-visible files, but remain inferred because a shared-worktree diff has no process ownership. Stop presents those paths conditionally: dispatch their configured review only when this session actually modified them according to its own tool activity, and ignore paths changed only by another session. |
