...
| (called via Bash, not a hook) | `file-checkpoint` | Snapshot, list and safely complete the pending queue. The shared `audit-files` skill is its caller on both hosts; completion removes only paths whose contents still match its token. |
