---
name: settings
description: "View and change Guard settings for this project, including user-configured plan, turn, and answer reviewers and edited-file review rules. Use when the user wants to configure audits, file review actions, reference paths, or deployment knowledge. Claude Code only."
...
Show and change **guard's** settings for this project. This includes choosing file
...
Always pass `--session` so queue changes and the automatic plan-gate switch also
...
| `turn_review` | JSON object, or `{}` | The user-supplied turn reviewer: `{"kind":"agent" or "skill","name":"..."}`. Unset or `{}` means no turn review. `audit-turn` invokes only this reviewer, without an additional switch. |
...
Use this flow when the user asks for file review by folder or file pattern. First show
current settings, then explain the existing built-in actions before asking the user to
supply a custom reviewer:

- `agent:guard:comment-corrector` checks source comments and edits them in place; say so when selecting it.
...
Inspect only the project's relevant file paths when needed to turn the request into patterns.
Use the configured reference directory for reference rules; do not assume a directory
layout. Source code, configuration files, instruction files, and reference Markdown use the
same rule selection. These examples illustrate the rule structure, not a default configuration:
...
`files_exclude`, for example `["docs/generated/**", "**/draft-*.md"]`; `action: null` is invalid.
...
An exact tie uses the earlier rule. Unmatched paths are not reviewed. `files_exclude` applies
to every file type, including source code and saved references. `doc_dir` limits ordinary
Markdown coverage.
...
requested lists through the CLI. Matching rules are sufficient to select files.
If a write fails, report which changes succeeded and stop. Example commands:

```sh
GUARD_SETTINGS_SKILL=1 "${CLAUDE_PLUGIN_ROOT}/scripts/guard_hook.py" settings set file_review_rules '<complete-compact-json-array>' --session "${CLAUDE_SESSION_ID}"
GUARD_SETTINGS_SKILL=1 "${CLAUDE_PLUGIN_ROOT}/scripts/guard_hook.py" settings set files_exclude 'docs/generated/**,!docs/generated/README.md,**/draft-*.md' --session "${CLAUDE_SESSION_ID}"
```

Pass the JSON as one safely quoted shell argument without interpolation or shell evaluation.
`files_exclude` and `doc_dir` take comma-separated lists at the CLI, not JSON
arrays. Re-run `settings show` and report the resulting configuration. Saving settings does
not run reviews; the user runs `/guard:audit-files` when the pending edits are ready.

There are no file-review switches. A matching `file_review_rules` entry selects a file for
an explicit checkpoint; an empty list disables file review. Never register
`guard:audit-files` as its own reviewer. The reference-index gate remains independent.

`doc_dir` scopes ordinary Markdown only. `files_exclude` is the single exclusion setting
and applies to all file types. Use `sources/**` to exclude an entire tree, including code,
or `sources/**/*.md` to exclude only its Markdown files. A later
`!sources/**/src/**/*.java` restores matching Java files; a still later exclusion can remove
them again. Restored files still require a matching `file_review_rules` entry. Preserve list
order, including repeated patterns. An exception can restore a file beneath an excluded
tree directly; Guard does not implement the full `.gitignore` syntax.
