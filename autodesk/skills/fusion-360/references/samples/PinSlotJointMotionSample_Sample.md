...
extern "C" XI_EXPORT bool run(const char* context)
{
    ...
    // Set the joint input
    jointInput->setAsPinSlotJointMotion(JointDirections::ZAxisJointDirection, JointDirections::XAxisJointDirection);
    ...
    Ptr<PinSlotJointMotion> pinSlotMotion = joint->jointMotion();
    if (!pinSlotMotion)
        return false;
    Ptr<JointLimits> limits = pinSlotMotion->rotationLimits();
    if (!limits)
        return false;
    limits->isRestValueEnabled(true);
    limits->restValue(1.0);
