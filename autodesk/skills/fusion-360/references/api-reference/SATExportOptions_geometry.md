...
"sATExportOptions\_var" is a variable referencing a SATExportOptions object. ```` ``` #include <Fusion/Fusion/SATExportOptions.h>  // Get the value of the property. Ptr<Base> propertyValue = sATExportOptions_var->geometry();  // Set the value of the property, where value_var is a Base. bool returnValue = sATExportOptions_var->geometry(value_var); ``` ```` |
