...
"scriptInput\_var" is a variable referencing a ScriptInput object. ```` ``` #include <Core/Application/ScriptInput.h>  // Get the value of the property. string propertyValue = scriptInput_var->version();  // Set the value of the property, where value_var is a string. bool returnValue = scriptInput_var->version(value_var); ``` ```` |
