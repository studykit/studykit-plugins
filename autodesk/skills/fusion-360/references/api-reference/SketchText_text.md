...
"sketchText\_var" is a variable referencing a SketchText object. ```` ``` #include <Fusion/Sketch/SketchText.h>  // Get the value of the property. string propertyValue = sketchText_var->text();  // Set the value of the property, where value_var is a string. bool returnValue = sketchText_var->text(value_var); ``` ```` |
