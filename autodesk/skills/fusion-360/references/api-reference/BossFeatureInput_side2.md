...
"bossFeatureInput\_var" is a variable referencing a BossFeatureInput object. ```` ``` #include <Fusion/Plastic/BossFeatureInput.h>  // Get the value of the property. Ptr<BossFeatureSideInput> propertyValue = bossFeatureInput_var->side2();  // Set the value of the property, where value_var is a BossFeatureSideInput. bool returnValue = bossFeatureInput_var->side2(value_var); ``` ```` |
