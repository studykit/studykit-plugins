...
"plasticRule\_var" is a variable referencing a PlasticRule object. ```` ``` #include <Fusion/Plastic/PlasticRule.h>  // Get the value of the property. double propertyValue = plasticRule_var->minimumDraftAngle();  // Set the value of the property, where value_var is a double. bool returnValue = plasticRule_var->minimumDraftAngle(value_var); ``` ```` |
