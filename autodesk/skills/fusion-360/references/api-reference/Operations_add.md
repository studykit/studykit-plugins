...
"operations\_var" is a variable referencing an [Operations](Operations.htm) object.```` ``` returnValue = operations_var.add(input) ``` ```` |
...
| --- | --- |
| Type | Description |
| [OperationBase](OperationBase.htm) | Returns the newly created Operation. |
