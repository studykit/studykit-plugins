...
"dXFFlatPatternExportOptions\_var" is a variable referencing a DXFFlatPatternExportOptions object. ```` ``` #include <Fusion/Fusion/DXFFlatPatternExportOptions.h>  // Get the value of the property. boolean propertyValue = dXFFlatPatternExportOptions_var->isExtentLinesExported();  // Set the value of the property, where value_var is a boolean. bool returnValue = dXFFlatPatternExportOptions_var->isExtentLinesExported(value_var); ``` ```` |
