...
"pocketRecognitionSelection\_var" is a variable referencing a PocketRecognitionSelection object. ```` ``` #include <Cam/GeometrySelections/PocketRecognitionSelection.h>  // Get the value of the property. double propertyValue = pocketRecognitionSelection_var->minimumPocketDepth();  // Set the value of the property, where value_var is a double. bool returnValue = pocketRecognitionSelection_var->minimumPocketDepth(value_var); ``` ```` |
