...
"operationBase\_var" is a variable referencing an OperationBase object. ```` ``` #include <Cam/Operations/OperationBase.h>  // Get the value of the property. boolean propertyValue = operationBase_var->isLightBulbOn();  // Set the value of the property, where value_var is a boolean. bool returnValue = operationBase_var->isLightBulbOn(value_var); ``` ```` |
