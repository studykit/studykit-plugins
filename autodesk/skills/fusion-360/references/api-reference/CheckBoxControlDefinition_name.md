...
"checkBoxControlDefinition\_var" is a variable referencing a CheckBoxControlDefinition object. ```` ``` #include <Core/UserInterface/CheckBoxControlDefinition.h>  // Get the value of the property. string propertyValue = checkBoxControlDefinition_var->name();  // Set the value of the property, where value_var is a string. bool returnValue = checkBoxControlDefinition_var->name(value_var); ``` ```` |
