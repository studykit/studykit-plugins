...
| --- | --- |
| Name | Description |
| [add](HTMLEvent_add.htm) | Add a handler to be notified when the event occurs. |
| [classType](HTMLEvent_classType.htm) | Static function that all classes support that returns the type of the class as a string. The returned string matches the string returned by the objectType property. For example if you have a reference to an object and you want to check if it's a SketchLine you can use myObject.objectType == fusion.SketchLine.classType(). |
| [remove](HTMLEvent_remove.htm) | Removes a handler from the event. |
...
| --- | --- |
| Name | Description |
| [isValid](HTMLEvent_isValid.htm) | Indicates if this object is still valid, i.e. hasn't been deleted or some other action done to invalidate the reference. |
| [name](HTMLEvent_name.htm) | The name of the event - e.g. "DocumentOpening" |
| [objectType](HTMLEvent_objectType.htm) | This property is supported by all objects in the API and returns a string that contains the full name (namespace::objecttype) describing the type of the object. |
| [sender](HTMLEvent_sender.htm) | The object that is firing the event. For example, in the case of a command input event this will return the command. |

## Accessed From

[Command.incomingFromHTML](Command_incomingFromHTML.htm), [Palette.incomingFromHTML](Palette_incomingFromHTML.htm), [TextCommandPalette.incomingFromHTML](TextCommandPalette_incomingFromHTML.htm)
