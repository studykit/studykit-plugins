...
"generateToolpathFuture\_var" is a variable referencing a GenerateToolpathFuture object. ```` ``` #include <Cam/CAM/GenerateToolpathFuture.h>  // Get the value of the property. boolean propertyValue = generateToolpathFuture_var->isGenerationCompleted(); ``` ```` |
