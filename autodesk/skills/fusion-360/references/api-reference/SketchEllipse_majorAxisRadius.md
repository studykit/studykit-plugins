...
"sketchEllipse\_var" is a variable referencing a SketchEllipse object. ```` ``` #include <Fusion/Sketch/SketchEllipse.h>  // Get the value of the property. double propertyValue = sketchEllipse_var->majorAxisRadius();  // Set the value of the property, where value_var is a double. bool returnValue = sketchEllipse_var->majorAxisRadius(value_var); ``` ```` |
