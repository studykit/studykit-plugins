...
"fusionUnitsManager\_var" is a variable referencing a [FusionUnitsManager](FusionUnitsManager.htm) object.  ```` ``` #include <Fusion/Fusion/FusionUnitsManager.h>  // Uses no optional arguments. returnValue = fusionUnitsManager_var->standardizeExpression(expression);  // Uses optional arguments. returnValue = fusionUnitsManager_var->standardizeExpression(expression, units); ``` ```` |
