...
"operationInput\_var" is a variable referencing an OperationInput object. ```` ``` #include <Cam/Operations/OperationInput.h>  // Get the value of the property. string propertyValue = operationInput_var->displayName();  // Set the value of the property, where value_var is a string. bool returnValue = operationInput_var->displayName(value_var); ``` ```` |
