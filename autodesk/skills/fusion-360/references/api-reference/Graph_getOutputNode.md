# Graph.getOutputNode Method![](../images/TestTubeLarge.png)
...
"graph\_var" is a variable referencing a [Graph](Graph.htm) object.```` ``` returnValue = graph_var.getOutputNode(outputNodeType) ``` ```` |
...
| --- | --- |
| Type | Description |
| [GraphNode](GraphNode.htm) | The output node if present in the graph, null otherwise. |
...
| outputNodeType | [GraphOutputNodeTypes](GraphOutputNodeTypes.htm) | The type of the output graph node. Not every graph has every type; the primary graph has all four ouput node types, the cell graph only has OutputDensity. |
