...
"floatParameterValue\_var" is a variable referencing a FloatParameterValue object. ```` ``` #include <Cam/Operations/FloatParameterValue.h>  // Get the value of the property. double propertyValue = floatParameterValue_var->value();  // Set the value of the property, where value_var is a double. bool returnValue = floatParameterValue_var->value(value_var); ``` ```` |
