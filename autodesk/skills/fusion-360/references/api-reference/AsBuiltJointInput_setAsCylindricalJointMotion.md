...
"asBuiltJointInput\_var" is a variable referencing an [AsBuiltJointInput](AsBuiltJointInput.htm) object.  ```` ``` #include <Fusion/Components/AsBuiltJointInput.h>  // Uses no optional arguments. returnValue = asBuiltJointInput_var->setAsCylindricalJointMotion(rotationAxis);  // Uses optional arguments. returnValue = asBuiltJointInput_var->setAsCylindricalJointMotion(rotationAxis, customRotationAxisEntity); ``` ```` |
