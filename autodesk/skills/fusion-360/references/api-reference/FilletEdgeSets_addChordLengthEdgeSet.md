...
"filletEdgeSets\_var" is a variable referencing a [FilletEdgeSets](FilletEdgeSets.htm) object.```` ``` returnValue = filletEdgeSets_var.addChordLengthEdgeSet(entities, chordLength, isTangentChain) ``` ```` |

"filletEdgeSets\_var" is a variable referencing a [FilletEdgeSets](FilletEdgeSets.htm) object.  ```` ``` #include <Fusion/Features/FilletEdgeSets.h>  returnValue = filletEdgeSets_var->addChordLengthEdgeSet(entities, chordLength, isTangentChain); ``` ```` |
...
| --- | --- |
| Type | Description |
| [ChordLengthFilletEdgeSet](ChordLengthFilletEdgeSet.htm) | Returns the newly created FilletEdgeSet. |
