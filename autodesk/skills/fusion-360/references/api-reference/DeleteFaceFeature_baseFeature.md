...
"deleteFaceFeature\_var" is a variable referencing a DeleteFaceFeature object. ```` ``` #include <Fusion/Features/DeleteFaceFeature.h>  // Get the value of the property. Ptr<BaseFeature> propertyValue = deleteFaceFeature_var->baseFeature(); ``` ```` |
