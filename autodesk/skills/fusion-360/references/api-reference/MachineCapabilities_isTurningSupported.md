...
"machineCapabilities\_var" is a variable referencing a MachineCapabilities object. ```` ``` #include <Cam/Machine/MachineCapabilities.h>  // Get the value of the property. boolean propertyValue = machineCapabilities_var->isTurningSupported();  // Set the value of the property, where value_var is a boolean. bool returnValue = machineCapabilities_var->isTurningSupported(value_var); ``` ```` |
