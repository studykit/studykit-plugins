...
"geometrySelection\_var" is a variable referencing a GeometrySelection object. ```` ``` #include <Cam/GeometrySelections/GeometrySelection.h>  // Get the value of the property. std::vector<Ptr<Base>> propertyValue = geometrySelection_var->value(); ``` ```` |
