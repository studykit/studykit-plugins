...
"setup\_var" is a variable referencing a Setup object. ```` ``` #include <Cam/CAM/Setup.h>  // Get the value of the property. string propertyValue = setup_var->name();  // Set the value of the property, where value_var is a string. bool returnValue = setup_var->name(value_var); ``` ```` |
