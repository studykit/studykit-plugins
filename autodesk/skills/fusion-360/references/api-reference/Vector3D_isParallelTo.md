...
"vector3D\_var" is a variable referencing a [Vector3D](Vector3D.htm) object.```` ``` returnValue = vector3D_var.isParallelTo(vector) ``` ```` |
...
| vector | [Vector3D](Vector3D.htm) | The vector to test parallelism to. |
