...
"radialMarkingMenu\_var" is a variable referencing a RadialMarkingMenu object.  ```` ``` # Get the value of the property. propertyValue = radialMarkingMenu_var.westCommand  # Set the value of the property. radialMarkingMenu_var.westCommand = propertyValue ``` ```` |

"radialMarkingMenu\_var" is a variable referencing a RadialMarkingMenu object. ```` ``` #include <Core/UserInterface/RadialMarkingMenu.h>  // Get the value of the property. Ptr<Base> propertyValue = radialMarkingMenu_var->westCommand();  // Set the value of the property, where value_var is a Base. bool returnValue = radialMarkingMenu_var->westCommand(value_var); ``` ```` |
