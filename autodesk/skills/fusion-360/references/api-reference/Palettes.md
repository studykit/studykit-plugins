...
| --- | --- |
| Name | Description |
| [add](Palettes_add.htm) | Creates a new Palette. |
| [add2](Palettes_add2.htm) | Creates a new Palette. |
| [addTransparent](Palettes_addTransparent.htm) | Creates a new transparent Palette. |
| [classType](Palettes_classType.htm) | Static function that all classes support that returns the type of the class as a string. The returned string matches the string returned by the objectType property. For example if you have a reference to an object and you want to check if it's a SketchLine you can use myObject.objectType == fusion.SketchLine.classType(). |
| [item](Palettes_item.htm) | Returns the specified palette using an index into the collection. |
| [itemById](Palettes_itemById.htm) | Returns the palette at the specified ID. |
...
| --- | --- |
| Name | Description |
| [count](Palettes_count.htm) | Gets the number of Palettes. |
| [isValid](Palettes_isValid.htm) | Indicates if this object is still valid, i.e. hasn't been deleted or some other action done to invalidate the reference. |
| [objectType](Palettes_objectType.htm) | This property is supported by all objects in the API and returns a string that contains the full name (namespace::objecttype) describing the type of the object. |

## Accessed From

[UserInterface.palettes](UserInterface_palettes.htm)
