...
"importManager\_var" is a variable referencing an [ImportManager](ImportManager.htm) object.```` ``` returnValue = importManager_var.createSTEPImportOptions(filename) ``` ```` |
...
| --- | --- |
| Type | Description |
| [STEPImportOptions](STEPImportOptions.htm) | The created STEPImportOptions object or null if the creation failed. |
