...
"palette\_var" is a variable referencing a Palette object. ```` ``` #include <Core/UserInterface/Palette.h>  // Get the value of the property. integer propertyValue = palette_var->height();  // Set the value of the property, where value_var is an integer. bool returnValue = palette_var->height(value_var); ``` ```` |
