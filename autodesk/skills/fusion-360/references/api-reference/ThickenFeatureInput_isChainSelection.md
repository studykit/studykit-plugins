...
"thickenFeatureInput\_var" is a variable referencing a ThickenFeatureInput object. ```` ``` #include <Fusion/Features/ThickenFeatureInput.h>  // Get the value of the property. boolean propertyValue = thickenFeatureInput_var->isChainSelection();  // Set the value of the property, where value_var is a boolean. bool returnValue = thickenFeatureInput_var->isChainSelection(value_var); ``` ```` |
