...
"baseFeature\_var" is a variable referencing a [BaseFeature](BaseFeature.htm) object.```` ``` returnValue = baseFeature_var.createForAssemblyContext(occurrence) ``` ```` |
...
| --- | --- |
| Type | Description |
| [BaseFeature](BaseFeature.htm) | Returns the proxy object or null if this is not the NativeObject. |
