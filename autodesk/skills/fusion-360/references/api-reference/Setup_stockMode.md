...
"setup\_var" is a variable referencing a Setup object. ```` ``` #include <Cam/CAM/Setup.h>  // Get the value of the property. SetupStockModes propertyValue = setup_var->stockMode();  // Set the value of the property, where value_var is a SetupStockModes. bool returnValue = setup_var->stockMode(value_var); ``` ```` |
