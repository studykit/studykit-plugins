...
"radialMarkingMenu\_var" is a variable referencing a RadialMarkingMenu object. |

"radialMarkingMenu\_var" is a variable referencing a RadialMarkingMenu object. ```` ``` #include <Core/UserInterface/RadialMarkingMenu.h>  // Get the value of the property. string propertyValue = radialMarkingMenu_var->text();  // Set the value of the property, where value_var is a string. bool returnValue = radialMarkingMenu_var->text(value_var); ``` ```` |
