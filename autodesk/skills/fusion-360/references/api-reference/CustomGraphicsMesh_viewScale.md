...
"customGraphicsMesh\_var" is a variable referencing a CustomGraphicsMesh object. ```` ``` #include <Fusion/Graphics/CustomGraphicsMesh.h>  // Get the value of the property. Ptr<CustomGraphicsViewScale> propertyValue = customGraphicsMesh_var->viewScale();  // Set the value of the property, where value_var is a CustomGraphicsViewScale. bool returnValue = customGraphicsMesh_var->viewScale(value_var); ``` ```` |
