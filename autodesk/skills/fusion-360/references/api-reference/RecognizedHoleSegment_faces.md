...
"recognizedHoleSegment\_var" is a variable referencing a RecognizedHoleSegment object. ```` ``` #include <Cam/HoleRecognition/RecognizedHoleSegment.h>  // Get the value of the property. std::vector<Ptr<Base>> propertyValue = recognizedHoleSegment_var->faces(); ``` ```` |
