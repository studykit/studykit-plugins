...
"machine\_var" is a variable referencing a Machine object. ```` ``` #include <Cam/Machine/Machine.h>  // Get the value of the property. string propertyValue = machine_var->model();  // Set the value of the property, where value_var is a string. bool returnValue = machine_var->model(value_var); ``` ```` |
