...
"chainSelection\_var" is a variable referencing a ChainSelection object. ```` ``` #include <Cam/GeometrySelections/ChainSelection.h>  // Get the value of the property. std::vector<Ptr<Base>> propertyValue = chainSelection_var->inputGeometry();  // Set the value of the property, where value_var is a Base. bool returnValue = chainSelection_var->inputGeometry(value_var); ``` ```` |
