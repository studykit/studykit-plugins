...
"sTLExportOptions\_var" is a variable referencing a STLExportOptions object. ```` ``` #include <Fusion/Fusion/STLExportOptions.h>  // Get the value of the property. double propertyValue = sTLExportOptions_var->surfaceDeviation();  // Set the value of the property, where value_var is a double. bool returnValue = sTLExportOptions_var->surfaceDeviation(value_var); ``` ```` |
