...
"setupInput\_var" is a variable referencing a SetupInput object. ```` ``` #include <Cam/CAM/SetupInput.h>  // Get the value of the property. Ptr<PrintSetting> propertyValue = setupInput_var->printSetting();  // Set the value of the property, where value_var is a PrintSetting. bool returnValue = setupInput_var->printSetting(value_var); ``` ```` |
