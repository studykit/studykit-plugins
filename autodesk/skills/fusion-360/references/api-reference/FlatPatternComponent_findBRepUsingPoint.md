...
"flatPatternComponent\_var" is a variable referencing a [FlatPatternComponent](FlatPatternComponent.htm) object.  ```` ``` #include <Fusion/SheetMetal/FlatPatternComponent.h>  // Uses no optional arguments. returnValue = flatPatternComponent_var->findBRepUsingPoint(point, entityType);  // Uses optional arguments. returnValue = flatPatternComponent_var->findBRepUsingPoint(point, entityType, proximityTolerance, visibleEntitiesOnly); ``` ```` |
