...
"sketchCircle\_var" is a variable referencing a [SketchCircle](SketchCircle.htm) object.  ```` ``` #include <Fusion/Sketch/SketchCircle.h>  // Uses no optional arguments. returnValue = sketchCircle_var->extend(endPoint);  // Uses optional arguments. returnValue = sketchCircle_var->extend(endPoint, createConstraints); ``` ```` |
