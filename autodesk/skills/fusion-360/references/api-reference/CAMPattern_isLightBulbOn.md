...
"cAMPattern\_var" is a variable referencing a CAMPattern object. ```` ``` #include <Cam/CAM/CAMPattern.h>  // Get the value of the property. boolean propertyValue = cAMPattern_var->isLightBulbOn();  // Set the value of the property, where value_var is a boolean. bool returnValue = cAMPattern_var->isLightBulbOn(value_var); ``` ```` |
