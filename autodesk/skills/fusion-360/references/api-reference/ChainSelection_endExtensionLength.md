...
"chainSelection\_var" is a variable referencing a ChainSelection object. ```` ``` #include <Cam/GeometrySelections/ChainSelection.h>  // Get the value of the property. double propertyValue = chainSelection_var->endExtensionLength();  // Set the value of the property, where value_var is a double. bool returnValue = chainSelection_var->endExtensionLength(value_var); ``` ```` |
