...
"setupInput\_var" is a variable referencing a SetupInput object. ```` ``` #include <Cam/CAM/SetupInput.h>  // Get the value of the property. Ptr<Machine> propertyValue = setupInput_var->machine();  // Set the value of the property, where value_var is a Machine. bool returnValue = setupInput_var->machine(value_var); ``` ```` |
