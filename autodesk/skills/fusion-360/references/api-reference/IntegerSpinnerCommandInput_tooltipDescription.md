...
"integerSpinnerCommandInput\_var" is a variable referencing an IntegerSpinnerCommandInput object. ```` ``` #include <Core/UserInterface/IntegerSpinnerCommandInput.h>  // Get the value of the property. string propertyValue = integerSpinnerCommandInput_var->tooltipDescription();  // Set the value of the property, where value_var is a string. bool returnValue = integerSpinnerCommandInput_var->tooltipDescription(value_var); ``` ```` |
