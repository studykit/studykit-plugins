...
"sketchLine\_var" is a variable referencing a [SketchLine](SketchLine.htm) object.  ```` ``` #include <Fusion/Sketch/SketchLine.h>  // Uses no optional arguments. returnValue = sketchLine_var->breakCurve(segmentPoint);  // Uses optional arguments. returnValue = sketchLine_var->breakCurve(segmentPoint, createConstraints); ``` ```` |
