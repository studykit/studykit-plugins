...
"component\_var" is a variable referencing a [Component](Component.htm) object.  ```` ``` #include <Fusion/Components/Component.h>  // Uses no optional arguments. returnValue = component_var->getPhysicalProperties();  // Uses optional arguments. returnValue = component_var->getPhysicalProperties(accuracy); ``` ```` |
