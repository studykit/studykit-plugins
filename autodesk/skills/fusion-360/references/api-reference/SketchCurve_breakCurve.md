...
"sketchCurve\_var" is a variable referencing a [SketchCurve](SketchCurve.htm) object.  ```` ``` #include <Fusion/Sketch/SketchCurve.h>  // Uses no optional arguments. returnValue = sketchCurve_var->breakCurve(segmentPoint);  // Uses optional arguments. returnValue = sketchCurve_var->breakCurve(segmentPoint, createConstraints); ``` ```` |
