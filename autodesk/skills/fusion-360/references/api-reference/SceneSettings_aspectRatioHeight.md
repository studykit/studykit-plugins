...
"sceneSettings\_var" is a variable referencing a SceneSettings object.  ```` ``` # Get the value of the property. propertyValue = sceneSettings_var.aspectRatioHeight  # Set the value of the property. sceneSettings_var.aspectRatioHeight = propertyValue ``` ```` |

"sceneSettings\_var" is a variable referencing a SceneSettings object. ```` ``` #include <Fusion/Render/SceneSettings.h>  // Get the value of the property. integer propertyValue = sceneSettings_var->aspectRatioHeight();  // Set the value of the property, where value_var is an integer. bool returnValue = sceneSettings_var->aspectRatioHeight(value_var); ``` ```` |
