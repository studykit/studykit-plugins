...
"offsetConstraint\_var" is a variable referencing an OffsetConstraint object. ```` ``` #include <Fusion/Sketch/OffsetConstraint.h>  // Get the value of the property. std::vector<Ptr<SketchCurve>> propertyValue = offsetConstraint_var->childCurves(); ``` ```` |
