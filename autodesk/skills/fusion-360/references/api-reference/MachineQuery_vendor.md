...
"machineQuery\_var" is a variable referencing a MachineQuery object. ```` ``` #include <Cam/Machine/MachineQuery.h>  // Get the value of the property. string propertyValue = machineQuery_var->vendor();  // Set the value of the property, where value_var is a string. bool returnValue = machineQuery_var->vendor(value_var); ``` ```` |
