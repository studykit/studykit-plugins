...
| --- | --- |
| Name | Description |
| [fusion360MachineFolder](CAMLibraryManager_fusion360MachineFolder.htm) | \*\*RETIRED\*\* Gets the absolute path to the folder containing Fusion machines. Fusion machines appear in the machine library under the Fusion tab. |
| [fusion360PostFolder](CAMLibraryManager_fusion360PostFolder.htm) | \*\*RETIRED\*\* Gets the absolute path to the folder containing Fusion posts. Fusion posts appear in the post library under the Fusion tab. |
| [isValid](CAMLibraryManager_isValid.htm) | Indicates if this object is still valid, i.e. hasn't been deleted or some other action done to invalidate the reference. |
| [localMachineFolder](CAMLibraryManager_localMachineFolder.htm) | \*\*RETIRED\*\* Gets the absolute path to the folder containing local machines. Local Machines appear in machine library under the local tab. |
| [localPostFolder](CAMLibraryManager_localPostFolder.htm) | \*\*RETIRED\*\* Gets the absolute path to the folder containing local posts. Local posts appear in post library under the local tab. |
| [machineLibrary](CAMLibraryManager_machineLibrary.htm) | The MachineLibrary provides access to machines. |
| [networkMachineFolder](CAMLibraryManager_networkMachineFolder.htm) | Gets the absolute path to the folder containing network machines. Network machines appear in the machine library under the network tab. |
| [objectType](CAMLibraryManager_objectType.htm) | This property is supported by all objects in the API and returns a string that contains the full name (namespace::objecttype) describing the type of the object. |
| [postLibrary](CAMLibraryManager_postLibrary.htm) | The PostLibrary provides access to postConfigurations. |
| [printSettingLibrary](CAMLibraryManager_printSettingLibrary.htm) | The PrintSettingLibrary provides access to print settings. |
| [stockMaterialLibrary](CAMLibraryManager_stockMaterialLibrary.htm) | ![Preview](../images/TestTubeSmall.png)The StockMaterialLibrary provides access to stock materials. You can only get a valid StockMaterialLibrary when you have access to Stock Materials private preview feature and enable the feature flag. |
| [templateLibrary](CAMLibraryManager_templateLibrary.htm) | The CAMTemplateLibrary provides access to templates |
| [toolLibraries](CAMLibraryManager_toolLibraries.htm) | The ToolLibraries provides access to tools. |

## Accessed From

[CAMManager.libraryManager](CAMManager_libraryManager.htm)
