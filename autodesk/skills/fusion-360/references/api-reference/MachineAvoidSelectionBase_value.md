...
"machineAvoidSelectionBase\_var" is a variable referencing a MachineAvoidSelectionBase object. ```` ``` #include <Cam/MachineAvoidSelections/MachineAvoidSelectionBase.h>  // Get the value of the property. std::vector<Ptr<Base>> propertyValue = machineAvoidSelectionBase_var->value(); ``` ```` |
