...
"color\_var" is a variable referencing a Color object. ```` ``` #include <Core/Application/Color.h>  // Get the value of the property. short propertyValue = color_var->opacity();  // Set the value of the property, where value_var is a short. bool returnValue = color_var->opacity(value_var); ``` ```` |
