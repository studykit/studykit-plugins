...
"sketchSelection\_var" is a variable referencing a SketchSelection object. ```` ``` #include <Cam/GeometrySelections/SketchSelection.h>  // Get the value of the property. std::vector<Ptr<Base>> propertyValue = sketchSelection_var->inputGeometry();  // Set the value of the property, where value_var is a Base. bool returnValue = sketchSelection_var->inputGeometry(value_var); ``` ```` |
