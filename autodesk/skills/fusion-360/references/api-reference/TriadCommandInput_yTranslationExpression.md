...
"triadCommandInput\_var" is a variable referencing a TriadCommandInput object. ```` ``` #include <Core/UserInterface/TriadCommandInput.h>  // Get the value of the property. string propertyValue = triadCommandInput_var->yTranslationExpression();  // Set the value of the property, where value_var is a string. bool returnValue = triadCommandInput_var->yTranslationExpression(value_var); ``` ```` |
