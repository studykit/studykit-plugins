...
"cAMHoleRecognition\_var" is a variable referencing a CAMHoleRecognition object. ```` ``` #include <Cam/CAM/CAMHoleRecognition.h>  // Get the value of the property. boolean propertyValue = cAMHoleRecognition_var->isSuppressed();  // Set the value of the property, where value_var is a boolean. bool returnValue = cAMHoleRecognition_var->isSuppressed(value_var); ``` ```` |
