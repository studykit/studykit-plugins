...
"circularPatternFeature\_var" is a variable referencing a CircularPatternFeature object. ```` ``` #include <Fusion/Features/CircularPatternFeature.h>  // Get the value of the property. Ptr<Occurrence> propertyValue = circularPatternFeature_var->assemblyContext(); ``` ```` |
