...
"printSettingLibrary\_var" is a variable referencing a [PrintSettingLibrary](PrintSettingLibrary.htm) object.```` ``` returnValue = printSettingLibrary_var.childPrintSettings(url) ``` ```` |
...
| url | [URL](URL.htm) | The URL of the folder to get PrintSettings from. |
