...
"zebraAnalysis\_var" is a variable referencing a ZebraAnalysis object. ```` ``` #include <Fusion/Fusion/ZebraAnalysis.h>  // Get the value of the property. string propertyValue = zebraAnalysis_var->name();  // Set the value of the property, where value_var is a string. bool returnValue = zebraAnalysis_var->name(value_var); ``` ```` |
