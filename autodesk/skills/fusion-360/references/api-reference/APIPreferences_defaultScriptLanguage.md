...
"aPIPreferences\_var" is a variable referencing an APIPreferences object. ```` ``` #include <Core/Application/APIPreferences.h>  // Get the value of the property. ProgrammingLanguages propertyValue = aPIPreferences_var->defaultScriptLanguage();  // Set the value of the property, where value_var is a ProgrammingLanguages. bool returnValue = aPIPreferences_var->defaultScriptLanguage(value_var); ``` ```` |
