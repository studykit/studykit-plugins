---
name: pdf-split
description: Split PDF files by bookmarks or page ranges using pypdf.
---
...
│     └─ uv run total_page.py input.pdf                    │
└────────────────────────┬─────────────────────────────────┘
                         ▼
┌──────────────────────────────────────────────────────────┐
│  2. Extract bookmark info (PDF page numbers)             │
│     └─ uv run get_bookmarks.py input.pdf                 │
└────────────────────────┬─────────────────────────────────┘
                         ▼
┌──────────────────────────────────────────────────────────┐
│  3. Map bookmark PDF pages ↔ TOC page numbers            │
│     └─ Use bookmark info to find TOC location            │
│     └─ Convert TOC pages to markdown for verification    │
│     └─ uv run pages_md.py input.pdf <toc-pages>          │
└────────────────────────┬─────────────────────────────────┘
                         ▼
┌──────────────────────────────────────────────────────────┐
│  4. Split by chapters                                    │
│     └─ Filename: TOC page numbers                        │
│     └─ split_by_page argument: PDF page numbers          │
└──────────────────────────────────────────────────────────┘
```
...
```bash
uv run /path/to/skills/pdf-split/scripts/total_page.py "<input.pdf>"
```
...
```bash
uv run /path/to/skills/pdf-split/scripts/get_bookmarks.py "<input.pdf>"
```
...
uv run /path/to/skills/pdf-split/scripts/pages_md.py "<input.pdf>" 3-14 -o toc.md
```
...
uv run /path/to/skills/pdf-split/scripts/pages_md.py "<input.pdf>" 15 | head -50
```
...
```bash
uv run /path/to/skills/pdf-split/scripts/pages_md.py "<input.pdf>" 1-30 -o front.md
```
...
uv run /path/to/skills/pdf-split/scripts/pages_md.py "<input.pdf>" 10-20 | grep -i "chapter 1"
```
...
uv run /path/to/skills/pdf-split/scripts/pages_md.py "<input.pdf>" 39 | head -50
```
...
**IMPORTANT**: Before splitting, check if `<filename>/` directory already exists. Skip if it exists.
...
uv run split_by_page.py "doc.pdf" "<filename>/001_Cover_p1.pdf" "1"
uv run split_by_page.py "doc.pdf" "<filename>/002_CONTENTS_p3-14.pdf" "3-14"
uv run split_by_page.py "doc.pdf" "<filename>/003_1_INTRODUCTION_p1-2.pdf" "15-16"
uv run split_by_page.py "doc.pdf" "<filename>/004_1.1_Uses_of_Computer_Networks_p2-11.pdf" "16-25"
uv run split_by_page.py "doc.pdf" "<filename>/005_1.2_Network_Hardware_p11-40.pdf" "25-54"
uv run split_by_page.py "doc.pdf" "<filename>/006_2_THE_PHYSICAL_LAYER_p41-42.pdf" "55-56"
uv run split_by_page.py "doc.pdf" "<filename>/007_2.1_Guided_Transmission_p42-60.pdf" "56-74"
```
...
uv run split_by_page.py "doc.pdf" "<filename>/001_Part_1_p1-20.pdf" "1-20"
uv run split_by_page.py "doc.pdf" "<filename>/002_Part_2_p21-40.pdf" "21-40"
uv run split_by_page.py "doc.pdf" "<filename>/003_Part_3_p41-60.pdf" "41-60"
```

### Page Range Formats

```bash
uv run /path/to/skills/pdf-split/scripts/split_by_page.py "<input.pdf>" "<output.pdf>" "<pages>"
```
...
- Dependencies are auto-installed via inline script metadata (PEP 723)
