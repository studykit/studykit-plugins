...
extern "C" XI_EXPORT bool run(const char* context)
{
    ...
    if (!centerPoint)
        return false;
    Ptr<SketchCircle> circle = sketchCircles->addByCenterRadius(centerPoint, 2.5);
    if (!circle)
        return false;
    ...
    // Create the joint geometry
    Ptr<JointGeometry> jointGeometry = JointGeometry::createByPlanarFace(endFace, nullptr, CenterKeyPoint);
    if (!jointGeometry)
        return false;
