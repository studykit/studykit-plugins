...
"sTEPExportOptions\_var" is a variable referencing a STEPExportOptions object. ```` ``` #include <Fusion/Fusion/STEPExportOptions.h>  // Get the value of the property. string propertyValue = sTEPExportOptions_var->filename();  // Set the value of the property, where value_var is a string. bool returnValue = sTEPExportOptions_var->filename(value_var); ``` ```` |
