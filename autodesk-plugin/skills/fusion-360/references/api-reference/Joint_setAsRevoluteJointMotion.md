...
"joint\_var" is a variable referencing a [Joint](Joint.htm) object.```` ``` # Uses no optional arguments. returnValue = joint_var.setAsRevoluteJointMotion(rotationAxis)  # Uses optional arguments. returnValue = joint_var.setAsRevoluteJointMotion(rotationAxis, customRotationAxisEntity) ``` ```` |

"joint\_var" is a variable referencing a [Joint](Joint.htm) object.  ```` ``` #include <Fusion/Components/Joint.h>  // Uses no optional arguments. returnValue = joint_var->setAsRevoluteJointMotion(rotationAxis);  // Uses optional arguments. returnValue = joint_var->setAsRevoluteJointMotion(rotationAxis, customRotationAxisEntity); ``` ```` |
