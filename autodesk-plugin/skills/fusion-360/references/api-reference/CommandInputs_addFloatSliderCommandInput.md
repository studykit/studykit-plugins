...
"commandInputs\_var" is a variable referencing a [CommandInputs](CommandInputs.htm) object.  ```` ``` #include <Core/UserInterface/CommandInputs.h>  // Uses no optional arguments. returnValue = commandInputs_var->addFloatSliderCommandInput(id, name, unitType, min, max);  // Uses optional arguments. returnValue = commandInputs_var->addFloatSliderCommandInput(id, name, unitType, min, max, hasTwoSliders); ``` ```` |
