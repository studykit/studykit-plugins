...
"saveImageFileOptions\_var" is a variable referencing a SaveImageFileOptions object. ```` ``` #include <Core/Application/SaveImageFileOptions.h>  // Get the value of the property. integer propertyValue = saveImageFileOptions_var->width();  // Set the value of the property, where value_var is an integer. bool returnValue = saveImageFileOptions_var->width(value_var); ``` ```` |
