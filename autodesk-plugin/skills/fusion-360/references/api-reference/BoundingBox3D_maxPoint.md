...
"boundingBox3D\_var" is a variable referencing a BoundingBox3D object. ```` ``` #include <Core/Geometry/BoundingBox3D.h>  // Get the value of the property. Ptr<Point3D> propertyValue = boundingBox3D_var->maxPoint();  // Set the value of the property, where value_var is a Point3D. bool returnValue = boundingBox3D_var->maxPoint(value_var); ``` ```` |
