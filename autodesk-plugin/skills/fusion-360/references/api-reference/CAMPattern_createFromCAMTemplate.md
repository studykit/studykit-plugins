...
"cAMPattern\_var" is a variable referencing a [CAMPattern](CAMPattern.htm) object.```` ``` returnValue = cAMPattern_var.createFromCAMTemplate(camTemplate) ``` ```` |

"cAMPattern\_var" is a variable referencing a [CAMPattern](CAMPattern.htm) object.  ```` ``` #include <Cam/CAM/CAMPattern.h>  returnValue = cAMPattern_var->createFromCAMTemplate(camTemplate); ``` ```` |
