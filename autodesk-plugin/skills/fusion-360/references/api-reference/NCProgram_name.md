...
"nCProgram\_var" is a variable referencing a NCProgram object. ```` ``` #include <Cam/NCProgram/NCProgram.h>  // Get the value of the property. string propertyValue = nCProgram_var->name();  // Set the value of the property, where value_var is a string. bool returnValue = nCProgram_var->name(value_var); ``` ```` |
