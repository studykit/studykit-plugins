...
"arc3D\_var" is a variable referencing an Arc3D object. ```` ``` #include <Core/Geometry/Arc3D.h>  // Get the value of the property. double propertyValue = arc3D_var->startAngle();  // Set the value of the property, where value_var is a double. bool returnValue = arc3D_var->startAngle(value_var); ``` ```` |
