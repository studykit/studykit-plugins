...
"sMTExportOptions\_var" is a variable referencing a SMTExportOptions object. ```` ``` #include <Fusion/Fusion/SMTExportOptions.h>  // Get the value of the property. Ptr<Base> propertyValue = sMTExportOptions_var->geometry();  // Set the value of the property, where value_var is a Base. bool returnValue = sMTExportOptions_var->geometry(value_var); ``` ```` |
