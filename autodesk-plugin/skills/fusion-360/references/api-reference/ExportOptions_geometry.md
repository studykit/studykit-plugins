...
"exportOptions\_var" is a variable referencing an ExportOptions object. ```` ``` #include <Fusion/Fusion/ExportOptions.h>  // Get the value of the property. Ptr<Base> propertyValue = exportOptions_var->geometry();  // Set the value of the property, where value_var is a Base. bool returnValue = exportOptions_var->geometry(value_var); ``` ```` |
