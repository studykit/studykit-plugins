...
"iGESExportOptions\_var" is a variable referencing an IGESExportOptions object. ```` ``` #include <Fusion/Fusion/IGESExportOptions.h>  // Get the value of the property. Ptr<Base> propertyValue = iGESExportOptions_var->geometry();  // Set the value of the property, where value_var is a Base. bool returnValue = iGESExportOptions_var->geometry(value_var); ``` ```` |
