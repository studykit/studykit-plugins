...
"sketchEllipticalArc\_var" is a variable referencing a SketchEllipticalArc object. ```` ``` #include <Fusion/Sketch/SketchEllipticalArc.h>  // Get the value of the property. double propertyValue = sketchEllipticalArc_var->minorAxisRadius();  // Set the value of the property, where value_var is a double. bool returnValue = sketchEllipticalArc_var->minorAxisRadius(value_var); ``` ```` |
