...
"fusionDocument\_var" is a variable referencing a FusionDocument object.  ```` ``` # Get the value of the property. propertyValue = fusionDocument_var.creationId ``` ```` |

"fusionDocument\_var" is a variable referencing a FusionDocument object. ```` ``` #include <Fusion/Fusion/FusionDocument.h>  // Get the value of the property. string propertyValue = fusionDocument_var->creationId(); ``` ```` |
