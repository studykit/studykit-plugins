...
"camera\_var" is a variable referencing a Camera object. ```` ``` #include <Core/Application/Camera.h>  // Get the value of the property. Ptr<Vector3D> propertyValue = camera_var->upVector();  // Set the value of the property, where value_var is a Vector3D. bool returnValue = camera_var->upVector(value_var); ``` ```` |
