...
"postConfigurationQuery\_var" is a variable referencing a PostConfigurationQuery object. ```` ``` #include <Cam/Post/PostConfigurationQuery.h>  // Get the value of the property. PostCapabilities propertyValue = postConfigurationQuery_var->capability();  // Set the value of the property, where value_var is a PostCapabilities. bool returnValue = postConfigurationQuery_var->capability(value_var); ``` ```` |

## Property Value

This is a read/write property whose value is a [PostCapabilities](PostCapabilities.htm).
