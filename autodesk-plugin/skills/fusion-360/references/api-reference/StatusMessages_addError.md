...
"statusMessages\_var" is a variable referencing a [StatusMessages](StatusMessages.htm) object.  ```` ``` #include <Core/Application/StatusMessages.h>  // Uses no optional arguments. returnValue = statusMessages_var->addError();  // Uses optional arguments. returnValue = statusMessages_var->addError(messageId, message); ``` ```` |
...
| --- | --- |
| Type | Description |
| [StatusMessage](StatusMessage.htm) | Returns true if the error message was successfully added. |
