...
"rectangularPatternConstraint\_var" is a variable referencing a RectangularPatternConstraint object. ```` ``` #include <Fusion/Sketch/RectangularPatternConstraint.h>  // Get the value of the property. boolean propertyValue = rectangularPatternConstraint_var->isSymmetricInDirectionOne();  // Set the value of the property, where value_var is a boolean. bool returnValue = rectangularPatternConstraint_var->isSymmetricInDirectionOne(value_var); ``` ```` |
