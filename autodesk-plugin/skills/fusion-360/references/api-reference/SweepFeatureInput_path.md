# SweepFeatureInput.path Property
...
"sweepFeatureInput\_var" is a variable referencing a SweepFeatureInput object. ```` ``` #include <Fusion/Features/SweepFeatureInput.h>  // Get the value of the property. Ptr<Path> propertyValue = sweepFeatureInput_var->path();  // Set the value of the property, where value_var is a Path. bool returnValue = sweepFeatureInput_var->path(value_var); ``` ```` |
