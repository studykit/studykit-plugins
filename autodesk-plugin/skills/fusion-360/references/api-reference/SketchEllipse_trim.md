...
"sketchEllipse\_var" is a variable referencing a [SketchEllipse](SketchEllipse.htm) object.  ```` ``` #include <Fusion/Sketch/SketchEllipse.h>  // Uses no optional arguments. returnValue = sketchEllipse_var->trim(segmentPoint);  // Uses optional arguments. returnValue = sketchEllipse_var->trim(segmentPoint, createConstraints); ``` ```` |
