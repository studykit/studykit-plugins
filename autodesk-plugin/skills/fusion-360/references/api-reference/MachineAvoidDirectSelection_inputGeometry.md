...
"machineAvoidDirectSelection\_var" is a variable referencing a MachineAvoidDirectSelection object. ```` ``` #include <Cam/MachineAvoidSelections/MachineAvoidDirectSelection.h>  // Get the value of the property. std::vector<Ptr<Base>> propertyValue = machineAvoidDirectSelection_var->inputGeometry();  // Set the value of the property, where value_var is a Base. bool returnValue = machineAvoidDirectSelection_var->inputGeometry(value_var); ``` ```` |
