# Features.stitchFeatures Property

Parent Object: [Features](Features.htm)
Defined in namespace "adsk::fusion" and the header file is <Fusion/Features/Features.h>

## Description

Returns the collection that provides access to the Stitch features within the component and supports the creation of new Stitch features.

## Syntax

* [Python](#Python)
* [C++](#C++)

"features\_var" is a variable referencing a Features object. |

"features\_var" is a variable referencing a Features object. ```` ``` #include <Fusion/Features/Features.h>  // Get the value of the property. Ptr<StitchFeatures> propertyValue = features_var->stitchFeatures(); ``` ```` |

## Property Value

This is a read only property whose value is a [StitchFeatures](StitchFeatures.htm).

## Samples

|  |  |
| --- | --- |
| Name | Description |
| [stitchFeatures.add](stitchFeatures_add_Sample.htm) | Demonstrates the stitchFeatures.add method. |

## Version

Introduced in version June 2015

---

|  |  |
| --- | --- |
| © Copyright 2025 Autodesk, Inc. | Comment on this page. |