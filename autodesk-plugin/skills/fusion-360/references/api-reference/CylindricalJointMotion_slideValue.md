...
"cylindricalJointMotion\_var" is a variable referencing a CylindricalJointMotion object. ```` ``` #include <Fusion/Components/CylindricalJointMotion.h>  // Get the value of the property. double propertyValue = cylindricalJointMotion_var->slideValue();  // Set the value of the property, where value_var is a double. bool returnValue = cylindricalJointMotion_var->slideValue(value_var); ``` ```` |
