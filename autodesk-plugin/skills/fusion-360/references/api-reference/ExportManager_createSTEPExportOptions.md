...
"exportManager\_var" is a variable referencing an [ExportManager](ExportManager.htm) object.  ```` ``` #include <Fusion/Fusion/ExportManager.h>  // Uses no optional arguments. returnValue = exportManager_var->createSTEPExportOptions(filename);  // Uses optional arguments. returnValue = exportManager_var->createSTEPExportOptions(filename, geometry); ``` ```` |
...
| --- | --- |
| Type | Description |
| [STEPExportOptions](STEPExportOptions.htm) | The created STEPExportOptions object or null if the creation failed. |
