...
"sketchFittedSpline\_var" is a variable referencing a [SketchFittedSpline](SketchFittedSpline.htm) object.  ```` ``` #include <Fusion/Sketch/SketchFittedSpline.h>  // Uses no optional arguments. returnValue = sketchFittedSpline_var->breakCurve(segmentPoint);  // Uses optional arguments. returnValue = sketchFittedSpline_var->breakCurve(segmentPoint, createConstraints); ``` ```` |
