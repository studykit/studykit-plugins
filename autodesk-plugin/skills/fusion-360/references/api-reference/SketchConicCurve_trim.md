...
"sketchConicCurve\_var" is a variable referencing a [SketchConicCurve](SketchConicCurve.htm) object.  ```` ``` #include <Fusion/Sketch/SketchConicCurve.h>  // Uses no optional arguments. returnValue = sketchConicCurve_var->trim(segmentPoint);  // Uses optional arguments. returnValue = sketchConicCurve_var->trim(segmentPoint, createConstraints); ``` ```` |
