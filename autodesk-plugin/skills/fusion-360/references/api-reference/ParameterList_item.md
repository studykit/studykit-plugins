...
"parameterList\_var" is a variable referencing a [ParameterList](ParameterList.htm) object.```` ``` returnValue = parameterList_var.item(index) ``` ```` |
...
| --- | --- |
| Type | Description |
| [Parameter](Parameter.htm) | Returns the specified item or null if an invalid index was specified. |
