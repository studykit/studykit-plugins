...
"matrix3D\_var" is a variable referencing a [Matrix3D](Matrix3D.htm) object.```` ``` returnValue = matrix3D_var.setToRotation(angle, axis, origin) ``` ```` |
...
| axis | [Vector3D](Vector3D.htm) | The axis of rotation. |
| origin | [Point3D](Point3D.htm) | The origin point of the axis of rotation. |
