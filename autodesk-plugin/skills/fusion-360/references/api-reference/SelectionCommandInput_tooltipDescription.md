...
"selectionCommandInput\_var" is a variable referencing a SelectionCommandInput object. ```` ``` #include <Core/UserInterface/SelectionCommandInput.h>  // Get the value of the property. string propertyValue = selectionCommandInput_var->tooltipDescription();  // Set the value of the property, where value_var is a string. bool returnValue = selectionCommandInput_var->tooltipDescription(value_var); ``` ```` |
