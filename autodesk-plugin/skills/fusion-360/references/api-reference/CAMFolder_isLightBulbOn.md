...
"cAMFolder\_var" is a variable referencing a CAMFolder object. ```` ``` #include <Cam/CAM/CAMFolder.h>  // Get the value of the property. boolean propertyValue = cAMFolder_var->isLightBulbOn();  // Set the value of the property, where value_var is a boolean. bool returnValue = cAMFolder_var->isLightBulbOn(value_var); ``` ```` |
