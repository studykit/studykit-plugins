...
"boundingBox2D\_var" is a variable referencing a BoundingBox2D object. ```` ``` #include <Core/Geometry/BoundingBox2D.h>  // Get the value of the property. Ptr<Point2D> propertyValue = boundingBox2D_var->maxPoint();  // Set the value of the property, where value_var is a Point2D. bool returnValue = boundingBox2D_var->maxPoint(value_var); ``` ```` |
