...
"commandControl\_var" is a variable referencing a CommandControl object.  ```` ``` # Get the value of the property. propertyValue = commandControl_var.isPromoted  # Set the value of the property. commandControl_var.isPromoted = propertyValue ``` ```` |

"commandControl\_var" is a variable referencing a CommandControl object. ```` ``` #include <Core/UserInterface/CommandControl.h>  // Get the value of the property. boolean propertyValue = commandControl_var->isPromoted();  // Set the value of the property, where value_var is a boolean. bool returnValue = commandControl_var->isPromoted(value_var); ``` ```` |
