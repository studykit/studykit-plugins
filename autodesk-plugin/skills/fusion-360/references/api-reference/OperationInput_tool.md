...
"operationInput\_var" is a variable referencing an OperationInput object. ```` ``` #include <Cam/Operations/OperationInput.h>  // Get the value of the property. Ptr<Tool> propertyValue = operationInput_var->tool();  // Set the value of the property, where value_var is a Tool. bool returnValue = operationInput_var->tool(value_var); ``` ```` |
