...
"holeFeatureInput\_var" is a variable referencing a [HoleFeatureInput](HoleFeatureInput.htm) object.  ```` ``` #include <Fusion/Features/HoleFeatureInput.h>  // Uses no optional arguments. returnValue = holeFeatureInput_var->setOneSideToExtent(toEntity, matchShape);  // Uses optional arguments. returnValue = holeFeatureInput_var->setOneSideToExtent(toEntity, matchShape, directionHint); ``` ```` |
