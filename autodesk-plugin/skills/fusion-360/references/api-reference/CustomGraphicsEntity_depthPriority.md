...
"customGraphicsEntity\_var" is a variable referencing a CustomGraphicsEntity object.  ```` ``` # Get the value of the property. propertyValue = customGraphicsEntity_var.depthPriority  # Set the value of the property. customGraphicsEntity_var.depthPriority = propertyValue ``` ```` |

"customGraphicsEntity\_var" is a variable referencing a CustomGraphicsEntity object. ```` ``` #include <Fusion/Graphics/CustomGraphicsEntity.h>  // Get the value of the property. integer propertyValue = customGraphicsEntity_var->depthPriority();  // Set the value of the property, where value_var is an integer. bool returnValue = customGraphicsEntity_var->depthPriority(value_var); ``` ```` |
