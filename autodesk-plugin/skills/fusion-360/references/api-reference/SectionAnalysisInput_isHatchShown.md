...
"sectionAnalysisInput\_var" is a variable referencing a SectionAnalysisInput object. ```` ``` #include <Fusion/Fusion/SectionAnalysisInput.h>  // Get the value of the property. boolean propertyValue = sectionAnalysisInput_var->isHatchShown();  // Set the value of the property, where value_var is a boolean. bool returnValue = sectionAnalysisInput_var->isHatchShown(value_var); ``` ```` |
