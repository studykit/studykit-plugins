...
"sketchCircle\_var" is a variable referencing a [SketchCircle](SketchCircle.htm) object.  ```` ``` #include <Fusion/Sketch/SketchCircle.h>  // Uses no optional arguments. returnValue = sketchCircle_var->trim(segmentPoint);  // Uses optional arguments. returnValue = sketchCircle_var->trim(segmentPoint, createConstraints); ``` ```` |
