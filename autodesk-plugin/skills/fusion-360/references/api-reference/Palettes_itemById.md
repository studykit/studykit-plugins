...
"palettes\_var" is a variable referencing a [Palettes](Palettes.htm) object.```` ``` returnValue = palettes_var.itemById(id) ``` ```` |
...
| --- | --- |
| Type | Description |
| [Palette](Palette.htm) | Returns the palette of the specified id or null if no palette has the specified id. |
