...
"ellipticalCylinder\_var" is a variable referencing an EllipticalCylinder object. ```` ``` #include <Core/Geometry/EllipticalCylinder.h>  // Get the value of the property. Ptr<Vector3D> propertyValue = ellipticalCylinder_var->axis();  // Set the value of the property, where value_var is a Vector3D. bool returnValue = ellipticalCylinder_var->axis(value_var); ``` ```` |
