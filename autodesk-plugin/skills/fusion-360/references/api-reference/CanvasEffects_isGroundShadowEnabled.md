...
"canvasEffects\_var" is a variable referencing a CanvasEffects object.  ```` ``` # Get the value of the property. propertyValue = canvasEffects_var.isGroundShadowEnabled  # Set the value of the property. canvasEffects_var.isGroundShadowEnabled = propertyValue ``` ```` |

"canvasEffects\_var" is a variable referencing a CanvasEffects object. ```` ``` #include <Core/Application/CanvasEffects.h>  // Get the value of the property. boolean propertyValue = canvasEffects_var->isGroundShadowEnabled();  // Set the value of the property, where value_var is a boolean. bool returnValue = canvasEffects_var->isGroundShadowEnabled(value_var); ``` ```` |
