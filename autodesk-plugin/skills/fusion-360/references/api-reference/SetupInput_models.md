...
"setupInput\_var" is a variable referencing a SetupInput object. ```` ``` #include <Cam/CAM/SetupInput.h>  // Get the value of the property. std::vector<Ptr<Base>> propertyValue = setupInput_var->models();  // Set the value of the property, where value_var is a Base. bool returnValue = setupInput_var->models(value_var); ``` ```` |
