...
"configurationAppearanceColumn\_var" is a variable referencing a [ConfigurationAppearanceColumn](ConfigurationAppearanceColumn.htm) object.```` ``` returnValue = configurationAppearanceColumn_var.getCellByRowId(rowId) ``` ```` |
...
| --- | --- |
| Type | Description |
| [ConfigurationAppearanceCell](ConfigurationAppearanceCell.htm) | Returns the specified cell if successful and null if the id is not found. |
