...
"curveSelection\_var" is a variable referencing a CurveSelection object. ```` ``` #include <Cam/GeometrySelections/CurveSelection.h>  // Get the value of the property. std::vector<Ptr<Base>> propertyValue = curveSelection_var->inputGeometry();  // Set the value of the property, where value_var is a Base. bool returnValue = curveSelection_var->inputGeometry(value_var); ``` ```` |
