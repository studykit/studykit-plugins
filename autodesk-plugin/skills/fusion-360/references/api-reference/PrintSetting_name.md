...
"printSetting\_var" is a variable referencing a PrintSetting object. ```` ``` #include <Cam/PrintSetting/PrintSetting.h>  // Get the value of the property. string propertyValue = printSetting_var->name();  // Set the value of the property, where value_var is a string. bool returnValue = printSetting_var->name(value_var); ``` ```` |
