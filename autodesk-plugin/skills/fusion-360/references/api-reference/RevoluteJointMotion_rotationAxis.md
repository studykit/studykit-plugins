...
"revoluteJointMotion\_var" is a variable referencing a RevoluteJointMotion object. ```` ``` #include <Fusion/Components/RevoluteJointMotion.h>  // Get the value of the property. JointDirections propertyValue = revoluteJointMotion_var->rotationAxis();  // Set the value of the property, where value_var is a JointDirections. bool returnValue = revoluteJointMotion_var->rotationAxis(value_var); ``` ```` |
