...
"sketchArc\_var" is a variable referencing a SketchArc object. ```` ``` #include <Fusion/Sketch/SketchArc.h>  // Get the value of the property. double propertyValue = sketchArc_var->radius();  // Set the value of the property, where value_var is a double. bool returnValue = sketchArc_var->radius(value_var); ``` ```` |
