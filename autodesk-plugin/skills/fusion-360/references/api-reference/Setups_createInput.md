...
"setups\_var" is a variable referencing a [Setups](Setups.htm) object.```` ``` returnValue = setups_var.createInput(type) ``` ```` |
...
| --- | --- |
| Type | Description |
| [SetupInput](SetupInput.htm) | Returns new SetupInput object. |
...
| type | [OperationTypes](OperationTypes.htm) | The type specifies the type of the setup that should be created. |
