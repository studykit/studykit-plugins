...
"camera\_var" is a variable referencing a Camera object. ```` ``` #include <Core/Application/Camera.h>  // Get the value of the property. Ptr<Point3D> propertyValue = camera_var->eye();  // Set the value of the property, where value_var is a Point3D. bool returnValue = camera_var->eye(value_var); ``` ```` |
