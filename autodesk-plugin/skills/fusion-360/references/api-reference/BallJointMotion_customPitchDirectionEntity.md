...
"ballJointMotion\_var" is a variable referencing a BallJointMotion object.  ```` ``` # Get the value of the property. propertyValue = ballJointMotion_var.customPitchDirectionEntity  # Set the value of the property. ballJointMotion_var.customPitchDirectionEntity = propertyValue ``` ```` |

"ballJointMotion\_var" is a variable referencing a BallJointMotion object. ```` ``` #include <Fusion/Components/BallJointMotion.h>  // Get the value of the property. Ptr<Base> propertyValue = ballJointMotion_var->customPitchDirectionEntity();  // Set the value of the property, where value_var is a Base. bool returnValue = ballJointMotion_var->customPitchDirectionEntity(value_var); ``` ```` |
