...
"pipeFeatureInput\_var" is a variable referencing a PipeFeatureInput object.  ```` ``` # Get the value of the property. propertyValue = pipeFeatureInput_var.distanceTwo  # Set the value of the property. pipeFeatureInput_var.distanceTwo = propertyValue ``` ```` |

"pipeFeatureInput\_var" is a variable referencing a PipeFeatureInput object. ```` ``` #include <Fusion/Features/PipeFeatureInput.h>  // Get the value of the property. Ptr<ValueInput> propertyValue = pipeFeatureInput_var->distanceTwo();  // Set the value of the property, where value_var is a ValueInput. bool returnValue = pipeFeatureInput_var->distanceTwo(value_var); ``` ```` |
