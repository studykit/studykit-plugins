...
"loftSection\_var" is a variable referencing a [LoftSection](LoftSection.htm) object.```` ``` returnValue = loftSection_var.setSmoothEndCondition(weight) ``` ```` |

"loftSection\_var" is a variable referencing a [LoftSection](LoftSection.htm) object.  ```` ``` #include <Fusion/Features/LoftSection.h>  returnValue = loftSection_var->setSmoothEndCondition(weight); ``` ```` |
