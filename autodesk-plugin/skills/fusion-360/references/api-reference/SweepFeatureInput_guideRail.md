...
"sweepFeatureInput\_var" is a variable referencing a SweepFeatureInput object. ```` ``` #include <Fusion/Features/SweepFeatureInput.h>  // Get the value of the property. Ptr<Path> propertyValue = sweepFeatureInput_var->guideRail();  // Set the value of the property, where value_var is a Path. bool returnValue = sweepFeatureInput_var->guideRail(value_var); ``` ```` |
