...
"customGraphicsGroup\_var" is a variable referencing a CustomGraphicsGroup object.  ```` ``` # Get the value of the property. propertyValue = customGraphicsGroup_var.depthPriority  # Set the value of the property. customGraphicsGroup_var.depthPriority = propertyValue ``` ```` |

"customGraphicsGroup\_var" is a variable referencing a CustomGraphicsGroup object. ```` ``` #include <Fusion/Graphics/CustomGraphicsGroup.h>  // Get the value of the property. integer propertyValue = customGraphicsGroup_var->depthPriority();  // Set the value of the property, where value_var is an integer. bool returnValue = customGraphicsGroup_var->depthPriority(value_var); ``` ```` |
