...
"distanceValueCommandInput\_var" is a variable referencing a DistanceValueCommandInput object.  ```` ``` # Get the value of the property. propertyValue = distanceValueCommandInput_var.minimumValue  # Set the value of the property. distanceValueCommandInput_var.minimumValue = propertyValue ``` ```` |

"distanceValueCommandInput\_var" is a variable referencing a DistanceValueCommandInput object. ```` ``` #include <Core/UserInterface/DistanceValueCommandInput.h>  // Get the value of the property. double propertyValue = distanceValueCommandInput_var->minimumValue();  // Set the value of the property, where value_var is a double. bool returnValue = distanceValueCommandInput_var->minimumValue(value_var); ``` ```` |
