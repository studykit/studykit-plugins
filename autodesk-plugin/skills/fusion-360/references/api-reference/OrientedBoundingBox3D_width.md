...
"orientedBoundingBox3D\_var" is a variable referencing an OrientedBoundingBox3D object. ```` ``` #include <Core/Geometry/OrientedBoundingBox3D.h>  // Get the value of the property. double propertyValue = orientedBoundingBox3D_var->width();  // Set the value of the property, where value_var is a double. bool returnValue = orientedBoundingBox3D_var->width(value_var); ``` ```` |
