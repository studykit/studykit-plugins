...
| --- | --- |
| Name | Description |
| [classType](StringParameterValue_classType.htm) | Static function that all classes support that returns the type of the class as a string. The returned string matches the string returned by the objectType property. For example if you have a reference to an object and you want to check if it's a SketchLine you can use myObject.objectType == fusion.SketchLine.classType(). |
...
| --- | --- |
| Name | Description |
| [isValid](StringParameterValue_isValid.htm) | Indicates if this object is still valid, i.e. hasn't been deleted or some other action done to invalidate the reference. |
| [objectType](StringParameterValue_objectType.htm) | This property is supported by all objects in the API and returns a string that contains the full name (namespace::objecttype) describing the type of the object. |
| [parent](StringParameterValue_parent.htm) | Get the parameter object that the value is associated with. |
| [value](StringParameterValue_value.htm) | Get or set the value of the parameter. |
