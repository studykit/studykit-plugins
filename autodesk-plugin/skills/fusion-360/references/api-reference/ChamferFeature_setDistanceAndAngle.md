...
"chamferFeature\_var" is a variable referencing a [ChamferFeature](ChamferFeature.htm) object.```` ``` returnValue = chamferFeature_var.setDistanceAndAngle(distance, angle) ``` ```` |

"chamferFeature\_var" is a variable referencing a [ChamferFeature](ChamferFeature.htm) object.  ```` ``` #include <Fusion/Features/ChamferFeature.h>  returnValue = chamferFeature_var->setDistanceAndAngle(distance, angle); ``` ```` |
