...
"fusionArchiveExportOptions\_var" is a variable referencing a FusionArchiveExportOptions object. ```` ``` #include <Fusion/Fusion/FusionArchiveExportOptions.h>  // Get the value of the property. Ptr<Base> propertyValue = fusionArchiveExportOptions_var->geometry();  // Set the value of the property, where value_var is a Base. bool returnValue = fusionArchiveExportOptions_var->geometry(value_var); ``` ```` |
