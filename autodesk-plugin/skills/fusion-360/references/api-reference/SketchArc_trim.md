...
"sketchArc\_var" is a variable referencing a [SketchArc](SketchArc.htm) object.  ```` ``` #include <Fusion/Sketch/SketchArc.h>  // Uses no optional arguments. returnValue = sketchArc_var->trim(segmentPoint);  // Uses optional arguments. returnValue = sketchArc_var->trim(segmentPoint, createConstraints); ``` ```` |
