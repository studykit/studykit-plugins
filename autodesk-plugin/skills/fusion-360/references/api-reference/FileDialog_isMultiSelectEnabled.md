...
"fileDialog\_var" is a variable referencing a FileDialog object. ```` ``` #include <Core/UserInterface/FileDialog.h>  // Get the value of the property. boolean propertyValue = fileDialog_var->isMultiSelectEnabled();  // Set the value of the property, where value_var is a boolean. bool returnValue = fileDialog_var->isMultiSelectEnabled(value_var); ``` ```` |
