...
"filletFeatures\_var" is a variable referencing a [FilletFeatures](FilletFeatures.htm) object.```` ``` returnValue = filletFeatures_var.item(index) ``` ```` |
...
| --- | --- |
| Type | Description |
| [FilletFeature](FilletFeature.htm) | Returns the specified item or null if an invalid index was specified. |
