...
"shellFeatureInput\_var" is a variable referencing a ShellFeatureInput object. ```` ``` #include <Fusion/Features/ShellFeatureInput.h>  // Get the value of the property. Ptr<Occurrence> propertyValue = shellFeatureInput_var->creationOccurrence();  // Set the value of the property, where value_var is an Occurrence. bool returnValue = shellFeatureInput_var->creationOccurrence(value_var); ``` ```` |
