...
"sketchSelection\_var" is a variable referencing a SketchSelection object. ```` ``` #include <Cam/GeometrySelections/SketchSelection.h>  // Get the value of the property. std::vector<Ptr<Base>> propertyValue = sketchSelection_var->value(); ``` ```` |
