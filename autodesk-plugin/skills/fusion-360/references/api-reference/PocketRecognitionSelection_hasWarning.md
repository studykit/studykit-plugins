...
"pocketRecognitionSelection\_var" is a variable referencing a PocketRecognitionSelection object. ```` ``` #include <Cam/GeometrySelections/PocketRecognitionSelection.h>  // Get the value of the property. boolean propertyValue = pocketRecognitionSelection_var->hasWarning(); ``` ```` |
