...
"palette\_var" is a variable referencing a Palette object. ```` ``` #include <Core/UserInterface/Palette.h>  // Get the value of the property. PaletteDockingStates propertyValue = palette_var->dockingState();  // Set the value of the property, where value_var is a PaletteDockingStates. bool returnValue = palette_var->dockingState(value_var); ``` ```` |
