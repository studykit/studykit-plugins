...
"sketchEllipticalArc\_var" is a variable referencing a [SketchEllipticalArc](SketchEllipticalArc.htm) object.  ```` ``` #include <Fusion/Sketch/SketchEllipticalArc.h>  // Uses no optional arguments. returnValue = sketchEllipticalArc_var->breakCurve(segmentPoint);  // Uses optional arguments. returnValue = sketchEllipticalArc_var->breakCurve(segmentPoint, createConstraints); ``` ```` |
