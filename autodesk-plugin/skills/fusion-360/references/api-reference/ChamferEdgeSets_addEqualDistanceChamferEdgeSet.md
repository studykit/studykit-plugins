...
"chamferEdgeSets\_var" is a variable referencing a [ChamferEdgeSets](ChamferEdgeSets.htm) object.```` ``` returnValue = chamferEdgeSets_var.addEqualDistanceChamferEdgeSet(edges, distance, isTangentChain) ``` ```` |

"chamferEdgeSets\_var" is a variable referencing a [ChamferEdgeSets](ChamferEdgeSets.htm) object.  ```` ``` #include <Fusion/Features/ChamferEdgeSets.h>  returnValue = chamferEdgeSets_var->addEqualDistanceChamferEdgeSet(edges, distance, isTangentChain); ``` ```` |
