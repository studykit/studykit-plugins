...
"baseComponent\_var" is a variable referencing a [BaseComponent](BaseComponent.htm) object.  ```` ``` #include <Fusion/Components/BaseComponent.h>  // Uses no optional arguments. returnValue = baseComponent_var->findBRepUsingRay(originPoint, rayDirection, entityType);  // Uses optional arguments. returnValue = baseComponent_var->findBRepUsingRay(originPoint, rayDirection, entityType, proximityTolerance, visibleEntitiesOnly, hitPoints); ``` ```` |
