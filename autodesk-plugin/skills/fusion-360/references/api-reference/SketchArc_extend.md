...
"sketchArc\_var" is a variable referencing a [SketchArc](SketchArc.htm) object.  ```` ``` #include <Fusion/Sketch/SketchArc.h>  // Uses no optional arguments. returnValue = sketchArc_var->extend(endPoint);  // Uses optional arguments. returnValue = sketchArc_var->extend(endPoint, createConstraints); ``` ```` |
