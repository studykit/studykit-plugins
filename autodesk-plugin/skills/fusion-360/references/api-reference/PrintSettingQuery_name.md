...
"printSettingQuery\_var" is a variable referencing a PrintSettingQuery object. ```` ``` #include <Cam/PrintSetting/PrintSettingQuery.h>  // Get the value of the property. string propertyValue = printSettingQuery_var->name();  // Set the value of the property, where value_var is a string. bool returnValue = printSettingQuery_var->name(value_var); ``` ```` |
