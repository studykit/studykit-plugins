...
"configurationMaterialColumn\_var" is a variable referencing a [ConfigurationMaterialColumn](ConfigurationMaterialColumn.htm) object.```` ``` returnValue = configurationMaterialColumn_var.getCellByRowName(rowName) ``` ```` |
...
| --- | --- |
| Type | Description |
| [ConfigurationMaterialCell](ConfigurationMaterialCell.htm) | Returns the specified cell if successful and null if the name is not found. |
