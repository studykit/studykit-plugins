...
"markingMenuEvent\_var" is a variable referencing a [MarkingMenuEvent](MarkingMenuEvent.htm) object.```` ``` returnValue = markingMenuEvent_var.add(handler) ``` ```` |
...
| handler | [MarkingMenuEventHandler](MarkingMenuEventHandler.htm) | The handler object to be called when this event is fired. |
