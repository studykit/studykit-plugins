...
"productUsageData\_var" is a variable referencing a ProductUsageData object. ```` ``` #include <Core/Application/ProductUsageData.h>  // Get the value of the property. boolean propertyValue = productUsageData_var->isGoogleAnalyticsTrackingEnabled();  // Set the value of the property, where value_var is a boolean. bool returnValue = productUsageData_var->isGoogleAnalyticsTrackingEnabled(value_var); ``` ```` |
