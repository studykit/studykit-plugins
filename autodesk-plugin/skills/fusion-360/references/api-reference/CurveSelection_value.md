...
"curveSelection\_var" is a variable referencing a CurveSelection object. ```` ``` #include <Cam/GeometrySelections/CurveSelection.h>  // Get the value of the property. std::vector<Ptr<Base>> propertyValue = curveSelection_var->value(); ``` ```` |
