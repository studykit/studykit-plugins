...
"configurationPropertyColumn\_var" is a variable referencing a [ConfigurationPropertyColumn](ConfigurationPropertyColumn.htm) object.```` ``` returnValue = configurationPropertyColumn_var.getCell(rowIndex) ``` ```` |
...
| --- | --- |
| Type | Description |
| [ConfigurationPropertyCell](ConfigurationPropertyCell.htm) | Returns the specified cell if successful and null if an invalid index was specified. |
