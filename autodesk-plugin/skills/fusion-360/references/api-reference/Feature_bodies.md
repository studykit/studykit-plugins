...
"feature\_var" is a variable referencing a Feature object.  ```` ``` # Get the value of the property. propertyValue = feature_var.bodies ``` ```` |

"feature\_var" is a variable referencing a Feature object. ```` ``` #include <Fusion/Features/Feature.h>  // Get the value of the property. Ptr<BRepBodies> propertyValue = feature_var->bodies(); ``` ```` |
