# Appearances.count Property

Parent Object: [Appearances](Appearances.htm)
Defined in namespace "adsk::core" and the header file is <Core/Materials/Appearances.h>

## Description

The number of Materials in the collection.

## Syntax

* [Python](#Python)
* [C++](#C++)

"appearances\_var" is a variable referencing an Appearances object. |

"appearances\_var" is a variable referencing an Appearances object. ```` ``` #include <Core/Materials/Appearances.h>  // Get the value of the property. uinteger propertyValue = appearances_var->count(); ``` ```` |

## Property Value

This is a read only property whose value is a uinteger.

## Version

Introduced in version August 2014

---

|  |  |
| --- | --- |
| © Copyright 2025 Autodesk, Inc. | Comment on this page. |