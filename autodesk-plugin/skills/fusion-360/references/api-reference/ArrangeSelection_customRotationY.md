...
"arrangeSelection\_var" is a variable referencing an ArrangeSelection object. ```` ``` #include <Cam/GeometrySelections/ArrangeSelection.h>  // Get the value of the property. double propertyValue = arrangeSelection_var->customRotationY();  // Set the value of the property, where value_var is a double. bool returnValue = arrangeSelection_var->customRotationY(value_var); ``` ```` |
