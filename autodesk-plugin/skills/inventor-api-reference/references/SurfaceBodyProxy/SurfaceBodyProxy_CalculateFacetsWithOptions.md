# SurfaceBodyProxy.CalculateFacetsWithOptions Method
...
SurfaceBodyProxy.**CalculateFacetsWithOptions**( ***ChordalTolerance*** As Double, ***Options*** As [NameValueMap](../NameValueMap/NameValueMap.md), ***VertexCount*** As Long, ***FacetCount*** As Long, ***VertexCoordinates***() As Double, ***NormalVectors***() As Double, ***VertexIndices***() As Long, [***TextureCoordinates***] As Variant )
...
| VertexCount | Long | Output Long that specifies the number of vertices in the facet mesh. |
| FacetCount | Long | Output Long that specifies the number facets in the mesh. |
...
| TextureCoordinates | Variant | Optional output array of Doubles that defines the texture coordinates that Inventor uses to map the render style texture to the face. |
