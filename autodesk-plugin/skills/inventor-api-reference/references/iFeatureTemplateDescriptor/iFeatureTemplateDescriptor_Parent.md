# iFeatureTemplateDescriptor.Parent Property

Parent Object: [iFeatureTemplateDescriptor](../iFeatureTemplateDescriptor/iFeatureTemplateDescriptor.md)

## Description

Property that returns the parent ComponentDefinition object.

## Syntax

iFeatureTemplateDescriptor.**Parent**() As [ComponentDefinition](../ComponentDefinition/ComponentDefinition.md)

## Property Value

This is a read only property whose value is a [ComponentDefinition](../ComponentDefinition/ComponentDefinition.md).

## Version

Introduced in version 5.3
