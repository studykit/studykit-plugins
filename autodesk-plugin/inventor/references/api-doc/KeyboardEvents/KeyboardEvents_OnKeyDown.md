...
Event that occurs when the user presses a button on the keyboard. Only the keys specified in the enumerator are handled through this event. The input signifies the hardware key that was actually depressed.

## Syntax

KeyboardEvents.**OnKeyDown**( ***Key*** As Long, ***ShiftKeys*** As [ShiftStateEnum](../ShiftStateEnum.md) )
