# Sketch.OffsetSketchEntitiesUsingPoint Method
...
The base sketch entity is determined as follows: \* If only one sketch entity needs to be offset, it will be treated as the base sketch entity. \* If a group of end-to-end connected entities need to be offset, the first entity in the group will be treated as the base sketch entity. If this method successfully offsets the specified input sketch entities, the newly created sketch entities are returned. In the case that a group of end-to-end connected entities need to be offset, the offset positions for all other connected sketch entities can be inferred after the base sketch entity has been offset to pass through the specified point. (A group of connected entities that need to be offset will all be offset by the same distance. Therefore, the offset distance between the original base sketch entity and its offset will also be used to offset all other connected sketch entities. The offset direction of the base sketch entity will be used to infer the offset directions for the connected entities).

## Syntax

Sketch.**OffsetSketchEntitiesUsingPoint**( ***SketchEntities*** As [ObjectCollection](../ObjectCollection/ObjectCollection.md), ***OffsetPoint*** As [Point2d](../Point2d/Point2d.md), [***IncludeConnectedEntities***] As Boolean, [***CreateOffsetConstraints***] As Boolean ) As [SketchEntitiesEnumerator](../SketchEntitiesEnumerator/SketchEntitiesEnumerator.md)
