...
7. **Refresh `Resume`.** Upsert the provider-backed `Resume` comment with `comment resume` (updates the existing one, or creates it when missing) — its body starts with the provider heading for Resume (`## Resume` on GitHub, `h2. Resume` on Jira): `Approach` summarises what landed, `Waiting for` is "user review/merge", `Open questions` contains only non-blocking follow-up questions, and `Next` is "review and merge". Do not put an unfinished required Acceptance Criterion in `Open questions`, `Deferred`, or follow-up text under an `implemented` report; if a required AC cannot be completed, stop with `paused` instead. Do not name the branch in the issue body; do not transition issue state — the user resolves the issue after merge.
