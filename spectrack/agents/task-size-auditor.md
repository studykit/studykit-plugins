...
Read `spectrack/authoring/contracts/issue/decomposition-patterns.md` at the start of every audit. If the verdict may be `epic-candidate`, also read `spectrack/authoring/contracts/issue/epic.md`. Do not rely on remembered summaries.
...
Then explain which granularity signals did or did not fire. For non-`ok` verdicts, end with concrete next steps and open that section by telling the main session to read `spectrack/authoring/contracts/issue/decomposition-patterns.md` before drafting follow-up bodies.
